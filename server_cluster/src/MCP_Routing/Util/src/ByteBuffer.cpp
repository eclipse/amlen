		...
		_position(0)
{
	...
	//		delete this;
	//		_capacity = 0;
	//		throw MCPRuntimeError("Out of memory", ISMRC_RtngOutOfMemory);
	//	}
...
ByteBuffer::ByteBuffer(const char* buffer, size_t length, bool readOnly, bool noCopy, bool owner)
        : _buffer(NULL)
{
...
void ByteBuffer::checkSpace4Write(size_t index, size_t dataLength)
{
	if (_readOnly)
	{
		...
		errorMsg << "IndexOutOfBoundsException: Trying to write " << (unsigned int) dataLength;
		errorMsg << " bytes from " << (unsigned int) index;
		errorMsg << " while this place is read only.";
		throw MCPIndexOutOfBoundsError(errorMsg.str(),ISMRC_ClusterInternalError);
	}

	if (_buffer == NULL)
	{
		throw MCPRuntimeError("Trying to write a NULL byte buffer", ISMRC_NullPointer);
	}
	...
	if (newBuffer == NULL)
	{
		throw MCPRuntimeError("Out of memory", ISMRC_AllocateError);
	}
...
void ByteBuffer::writeString(const String &str)
{
    ...
    memcpy(_buffer+_position, str.c_str(), length);
    _position += length;
}
...
{
    if (_capacity < _position)
    {
        throw MCPRuntimeError("ByteBuffer::readByteArray _capacity < _position");
    }
...
ByteBuffer_SPtr ByteBuffer::createReadOnlyByteBuffer(const char* buffer,
		size_type length, bool takeOwnership)
{
	ByteBuffer_SPtr retBuffer(new ByteBuffer(buffer, length, true, takeOwnership));
	return retBuffer;
}
...
{
	ByteBuffer_SPtr retBuffer(new ByteBuffer(buffer, length, false, false));
	return retBuffer;
}
...
uint32_t ByteBuffer::getCRCchecksum(uint32_t skipLastNbytes) const
{
	...
	if (_readOnly)
	{
		...
		if (len < 0)
			throw MCPRuntimeError("CRC with skipLastNbytes, on a buffer smaller then N bytes",ISMRC_Error);
	}
	else
	{
		len = _position;
...
ByteBufferReadOnlyWrapper::ByteBufferReadOnlyWrapper(const char* buffer, size_t length) :
		ByteBuffer(buffer, length,true,true,false)
{
}

ByteBufferReadOnlyWrapper::~ByteBufferReadOnlyWrapper(){}

boost::shared_ptr<ByteBufferReadOnlyWrapper> ByteBufferReadOnlyWrapper::createByteBufferReadOnlyWrapper(
			const char* buffer, size_type length)
{
	boost::shared_ptr<ByteBufferReadOnlyWrapper> rv(new ByteBufferReadOnlyWrapper(buffer,length));
	return rv;
}
