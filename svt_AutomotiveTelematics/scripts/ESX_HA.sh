...
scenario_set_name="ATELM Scenarios vCenter HA"
source ../scripts/commonFunctions.sh
source template5.sh

test_template_set_prefix "atelm_AT1_"
...
# setup ha
#----------------------------
printf -v tname "atelm_AT1_%00d" ${n}

xml[${n}]=${tname}
test_template_initialize_test "${xml[${n}]}"
scenario[${n}]="${xml[${n}]} - setup switch"
timeouts[${n}]=60

lc=0
#component[$((++lc))]="cAppDriverWait,m1,-e|${M1_TESTROOT}/scripts/svt-runBASH.sh,-o|-s|{S}|-q|{Q}|-e|/usr/bin/sshpass{S}-p{S}nimda{S}ssh{S}admin@10.10.174.2{S}{Q}enable;configure{S}terminal;interface{S}port{S}27-30;lacp{S}mode{S}active;lacp{S}key{S}435;port-channel{S}min-links{S}4;exit;exit;write;disable;show{S}interface{S}port{S}27-30{S}lacp{Q}"
#component[$((++lc))]="cAppDriverWait,m1,-e|${M1_TESTROOT}/scripts/svt-runBASH.sh,-o|-s|{S}|-q|{Q}|-e|/usr/bin/sshpass{S}-p{S}nimda{S}ssh{S}admin@10.10.174.2{S}{Q}enable;configure{S}terminal;interface{S}port{S}31-34;lacp{S}mode{S}active;lacp{S}key{S}438;port-channel{S}min-links{S}4;exit;exit;write;disable;show{S}interface{S}port{S}31-34{S}lacp{Q}"
...
component[$((++lc))]="cAppDriverWait,m1,-e|${M1_TESTROOT}/scripts/svt-runCLI.sh,-o|-s|{S}|-a|${A1_HOST}|-e|create{S}aggregate-interface{S}agg0;reset{S}member;member{S}eth2{S}eth3{S}eth4{S}eth5;userdata{S}internal_interface;aggregation-policy{S}lacp;lacp-selection-logic{S}count;lacp-rate{S}fast;transmit-hash-policy{S}layer3+4;{S}ip;{S}address{S}${A1_IPv4_AGG0}/23;exit;exit,"
component[$((++lc))]="cAppDriverWait,m1,-e|${M1_TESTROOT}/scripts/svt-runCLI.sh,-o|-s|{S}|-a|${A2_HOST}|-e|create{S}aggregate-interface{S}agg0;reset{S}member;member{S}eth2{S}eth3{S}eth4{S}eth5;userdata{S}internal_interface;aggregation-policy{S}lacp;lacp-selection-logic{S}count;lacp-rate{S}fast;transmit-hash-policy{S}layer3+4;{S}ip;{S}address{S}${A2_IPv4_AGG0}/23;exit;exit,"
...
#----------------------------
printf -v tname "atelm_AT1_%02d" ${n}
...
xml[$n]="atelm_AT1_0$n"
test_template_initialize_test "atelm_AT1_0$n"
scenario[$n]="atelm_AT1_0$n "
...
xml[${n}]=${tname}
test_template_initialize_test "${xml[${n}]}"
scenario[${n}]="${xml[${n}]} - unsetup switch"
timeouts[${n}]=60

lc=0
#component[$((++lc))]="cAppDriverWait,m1,-e|${M1_TESTROOT}/scripts/svt-runCLI.sh,-o|-s|{S}|-a|${A1_HOST}|-e|delete{S}aggregate-interface{S}agg0,"
#component[$((++lc))]="cAppDriverWait,m1,-e|${M1_TESTROOT}/scripts/svt-runCLI.sh,-o|-s|{S}|-a|${A2_HOST}|-e|delete{S}aggregate-interface{S}agg0,"
#component[$((++lc))]="cAppDriverWait,m1,-e|${M1_TESTROOT}/scripts/svt-runBASH.sh,-o|-s|{S}|-q|{Q}|-e|/usr/bin/sshpass{S}-p{S}nimda{S}ssh{S}admin@10.10.174.2{S}{Q}enable;configure{S}terminal;interface{S}port{S}27-34;lacp{S}mode{S}off;exit;exit;write;disable;show{S}interface{S}port{S}27-34{S}lacp{Q}"
