...
public class ISMQueueToMQQueue extends MessagingTest { //implements MessageListener {
	
	public ISMQueueToMQQueue(String clientID) {
		super(clientID);
	}
...
//	private Set<String> receivedMessages = new HashSet<String>();
	...
	private int messagesToSend =100;
	...
	public static void main(String[] args) {
		...
		try {
			test = new ISMQueueToMQQueue("ISMQ2MQQ");
...
//				timeLastCalled = System.currentTimeMillis();
				...
				try {
					while (true) {
					nextMessage = test.consumer.receive(10000);
					if (nextMessage ==null) {
						break;
					}
...
//					try {
//						Thread.sleep(1000);
//						logger.printString("waiting for messages: " + test.messageArrived + " arrived");
...
//			// consumer = sess.createDurableSubscriber(myTopic, "testSub");
			...
			consumer = sess.createConsumer(mqQueue);
//			consumer.setMessageListener(this);
			
			//Now we are ready to start receiving the messages. Hence start the connection
			conn.start();
			
			logger.printString("JMS Subscriber started and subscribed to MQ queue " + mqQueueName);
		} catch (JMSException e) {
			e.printStackTrace();
	...
	private void setupPublisherJMS () {
		...
		try {
			...
			ismQueue = session.createQueue(queueName);
			producer = session.createProducer(ismQueue);
