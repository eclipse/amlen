...
function MQTTClient(host, port, id) {
    ...
    this.ismclient.onMessageArrived = ((function(message) {
        ...
        // }
        // mqtt_clients[id - 1].addLogMsg("<span class='send'>&lt;&lt;&nbsp;[" +
        // message.replyto + "] " + replyMsg
        // + "</span>");
        // this.publishWithReply(message.replyto, replyMsg, null,
        // message.corrid);
        // }
    }));
}
...
MQTTClient.prototype.doPublish = function(topic, message, silent) {
    if (silent != true) {
        ...
        var pmessage = new Messaging.Message(newmessage);
        // var ptopic = this.ismclient.getTopic(topic);
...
MQTTClient.prototype.displayClient = function() {
    ...
    $("#" + "client" + this.id + "publish").click(function() {
        if (mqtt_clients[id - 1].connected == true) {
            mqtt_clients[id - 1].doPublish($("#client" + id + "pubtopic").val(), $("#client" + id + "pubmsg").val());
        }
    });
    $("#" + "client" + this.id + "publishobj").click(function() {
        if (mqtt_clients[id - 1].connected == true) {
            var pubObject = {};
            ...
            mqtt_clients[id - 1].doPublish($("#client" + id + "pubtopic").val(), pubObject);
        }
    });
    $("#" + "client" + this.id + "publisharray").click(function() {
        if (mqtt_clients[id - 1].connected == true) {
            // TODO: generate array
            var pubArray = Array();
            for ( var i = 1; i <= 4; i++) {
            ...
            mqtt_clients[id - 1].doPublish($("#client" + id + "pubtopic").val(), pubArray);
        }
    });
    /*
