...
def dumpThreadTraceData(outhdl, threadData, prefix):
    ...
    try:
      print >>outhdl,"NEWTHREAD-"+prefix+": ",threadData
      ...
      try:
          ...
          print "Using default threadHistoryBuffSize of 2048"
          threadHistoryBuffSize = 2048

      while keepGoing:
         if threadData['traceHistoryIdent'][currTracePoint] != 0:
             print >>outhdl, 'TP[{0},{1}]: {2} {3}'.format(threadData, currTracePoint,
                                                       threadData['traceHistoryIdent'][currTracePoint],
                                                       threadData['traceHistoryValue'][currTracePoint])
      ...
      print "Exception [%s] processing threadData for threadData at %s " % (type(e).__name__, threadData)
      print "Exception args: %s" % e.args
      
    print >>outhdl, "ENDTHREAD: ", threadData
    print >>outhdl, "\n\n"
