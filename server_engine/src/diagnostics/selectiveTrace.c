...
const funcSpec_t activateFunc[]   = { { '>', "ieiq_rehydrateMsg"}, // Entry to iest_rehydrateMessage
                                      {'\0', NULL               }  // <SENTINEL>
                                    };
// Trace lines that deactivate tracing
const funcSpec_t deactivateFunc[] = { { '<', "ieiq_rehydrateMsg"}, // Exit from iest_rehydrateMessage
                                      {'\0', NULL               }  // <SENTINEL>
                                    };
...
const funcSpec_t roundupFunc      =   { '<', "ism_engine_term"      }; // Exit from ism_engine_term

char *fixedFilename = DUMPPATH "selectiveTrace"; // NULL for separate file names by pid&tid

uint64_t g_numLines = 10000;
...
traceAction_t getTraceAction(const char *fmt, const char *funcName)
{
    traceAction_t action = noTrace;

    if (fmt != NULL && funcName != NULL)
    {
        const char formatChar = fmt[0];

        // Activate
        const funcSpec_t *checkFunc = activateFunc;

        while(checkFunc->funcName != NULL)
        {
            if (formatChar == checkFunc->formatChar && strcmp(funcName, checkFunc->funcName) == 0)
            {
                action = activate;
                goto mod_exit;
            }
        ...
        if (roundupFunc.funcName != NULL && formatChar == roundupFunc.formatChar && strcmp(funcName, roundupFunc.funcName))
        {
            action = roundup;
        }
    }
...
XAPI void ism_insertTrace(int level, int opt, const char * file, int lineno, const char * fmt, ...)
{
    ...
    int cur;
    int inlen  = 0;
    const char *possibleFunc = NULL;

    if (fmt)
    {
        va_start(arglist, fmt);
        possibleFunc = va_arg(arglist, const char *);
    }

    traceAction_t action = getTraceAction(fmt, possibleFunc);

    if (fmt) va_end(arglist);
...
{
    // Only interested if we're between our trace lines
    if (getTraceAction(NULL, NULL) == noTrace) return;
...
traceBuffer_t *allocateTraceBuffer(uint64_t numLines, uint32_t lineLength)
{
    traceBuffer_t *newBuffer = ism_common_calloc(ISM_MEM_PROBE(ism_memory_engine_misc,77),1, sizeof(traceBuffer_t) + (numLines * lineLength));
