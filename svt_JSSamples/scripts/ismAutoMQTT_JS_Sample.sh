...
#
# Description:
#   This script executes ISM test bucket for:  JavaScript/JSON(websocket) Client running the Sample Appl. driven by Selenium.
#
# Input Parameters:
...
# 3.1.1 Feature Test - Long client ID
run03Result=0
...
# 3.1.1 Feature Test - Zero length client ID
run04Result=0
...
# 3.1.1 Feature Test - Maximum length client ID
run05Result=0
...
# 3.1.1 IMA Feature Test - Shared subscriptions 
run06Result=0
...
###RUNCMD="../scripts/run-scenarios.sh  ism-MQTT_JS_Sample-runScenarios02.sh ${logfile}"
###echo Running command: ${RUNCMD}
###$RUNCMD
#run02Result=$?
...
exit $( expr ${run01Result} + ${run02Result} + ${run03Result} + ${run04Result} + ${run05Result} + ${run06Result} )
