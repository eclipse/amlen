...
/**
 * The intention of this class is to keep creating durable and non durable MQTT clients which then disconnect.
 * The idea is that this is run alongside the regression stress bucket so that we keep creating more
 * and more owner records of this type in the store.
 *
 */
public class CreateDisconnectedClients implements Runnable {
	...
	private String endpoint = "SVTDisconnectedClientsEP";
	private String connectionPolicy = "SVTDisconnectedClientsCP";
	private String messagingPolicy = "SVTDisconnectedClientsMP";
	private String messagingHub = "SVTDisconnectedClientsHub";
	...
	public CreateDisconnectedClients(String prefix, String ipaddress, String port, String cliaddress, int number, boolean tearDown, boolean traceFile) throws Exception
	{
		...
		if(traceFile)
		{
			File audit = new File("DisconnectedClients");
			if(!audit.exists())
			{
				audit.createNewFile();
			}
			trace = new Audit("DisconnectedClients", true);
		}
		this.setup();
	}
	
	public CreateDisconnectedClients(String prefix, String ipaddress, String port, String cliaddressA, String cliaddressB, int number, boolean tearDown, boolean traceFile) throws Exception
	{
		/*
	...
	public void run() {
		...
		{
			trace.trace("Starting to create clients with prefix " + clientID);
		}
		System.out.println("Starting to create clients with prefix " + clientID);
		...
		for(int i=0; i<numberToCreate; i++)
		{
			try
			{
				...
				if(trace!= null && trace.traceOn())
				{
					trace.trace("Created and disconnect " + clientID+i);
				}
