...
function print_help {
    ...
    echo "                                  Note that contents of ${IMA_SVR_DATA_PATH}/data and ${IMA_SVR_DATA_PATH}/diag/logs"
...
IS_DOCKER=0
ADDITIONAL_DIRECTORIES="/etc /var/log"
...
# 3. Locate imaserver directory
    IMASERVER_SOURCE=$(dirname "$(rpm -q --filesbypkg IBMWIoTPMessageGatewayServer 2> /dev/null | grep lib64 | head -n 1  | awk '{print $2}')" 2> /dev/null)
...
function collect_host_data {
    ...
    then
        ...
        for dr in /*; do grep -E " ${dr} " /proc/mounts > /dev/null || du -sh ${dr}; done
    fi
    echo "-------------------------------------"
    echo "5. locale"
    locale 2>&1
    ...
    echo "36. ps -ef"
    ps -ef 2>&1
    echo "37. ps aux --sort -rss"
    ps aux --sort -rss 2>&1
    echo "38. ps aux --sort -vsz"
    ps aux --sort -rss 2>&1
    echo "39. cat /proc/$(pgrep imaserver)/smaps 2>&1"
    cat /proc/$(pgrep imaserver)/smaps 2>&1
    echo "-------------------------------------"
    ...
    sysctl -a 2>&1
    echo "-------------------------------------"
    echo "41. cat /proc/cpuinfo"
    cat /proc/cpuinfo 2>&1
    echo "-------------------------------------"
    echo "42. cat /proc/meminfo"
    cat /proc/meminfo 2>&1
    echo "-------------------------------------"
    echo "43. cat /proc/mounts"
    cat /proc/mounts 2>&1
    echo "-------------------------------------"
    echo "44. cat /proc/cmdline"
    cat /proc/cmdline 2>&1
    echo "-------------------------------------"
    echo "45. cat /proc/misc"
    cat /proc/misc 2>&1
    echo "-------------------------------------"
    echo "46. cat /proc/locks"
    cat /proc/locks 2>&1
    echo "-------------------------------------"
    echo "47. cat /proc/mtrr"
    cat /proc/mtrr 2>&1
    echo "-------------------------------------"
    echo "48. cat /proc/partitions"
    cat /proc/partitions 2>&1
    echo "-------------------------------------"
    echo "49. cat /proc/iomem"
    cat /proc/iomem 2>&1
    echo "-------------------------------------"
    echo "50. cat /proc/ioports"
    cat /proc/ioports 2>&1
    echo "-------------------------------------"
    echo "51. cat /proc/modules"
    cat /proc/modules 2>&1
    echo "-------------------------------------"
    echo "52. cat /proc/net/sockstat"
    cat /proc/net/sockstat
    echo "-------------------------------------"

    for i in {1..5}
    do
        echo "53. cat /proc/interrupts [$i]"
        cat /proc/interrupts 2>&1
        echo "-------------------------------------"
        echo "54. cat /proc/slabinfo [$i]"
        cat /proc/slabinfo 2>&1
        echo "-------------------------------------"
        echo "55. cat /proc/vmstat [$i]"
        cat /proc/vmstat 2>&1
        echo "-------------------------------------"
        sleep 2
    done
    ...
    then
        ...
        docker ps -a 2>&1
        echo "-------------------------------------"
        echo "57. docker images"
        docker images 2>&1
        echo "-------------------------------------"
        echo "58. memory cgroup"
...
then
    ...
    if [ ! -z "$WEBUI_SOURCE" ]
    then
        echo "Processing MessageSight installation in ${WEBUI_SOURCE}"
    fi
fi
...
then
    ...
    then
        ...
        if ! docker exec -it "$CONTAINER_ID" ${IMA_SVR_INSTALL_PATH}/bin/internal_mustgather.sh > "${WORK_DIR}"/docker_mustgather.txt 2>&1
        then
            docker exec -it "$CONTAINER_ID" ${IMA_WEBUI_INSTALL_PATH}/bin/internal_mustgather.sh > "${WORK_DIR}"/docker_mustgather.txt 2>&1
        fi
        docker cp "${CONTAINER_ID}":${IMA_SVR_DATA_PATH}/diag/cores/container_mustgather.tar.gz . 2> /dev/null
    else
        ...
        then
            ...
            if ! docker run "$ADDITIONAL_PARMS" --volumes-from "$CONTAINER_ID" --entrypoint=${IMA_SVR_INSTALL_PATH}/bin/internal_mustgather.sh --name "$NEW_NAME" "$DEBUG_IMAGE_NAME" > "${WORK_DIR}"/docker_mustgather.txt 2>&1
            then
                if ! docker run "$ADDITIONAL_PARMS" --volumes-from "$CONTAINER_ID" --entrypoint=${IMA_WEBUI_INSTALL_PATH}/bin/internal_mustgather.sh --name "$NEW_NAME" "$DEBUG_IMAGE_NAME" > "${WORK_DIR}"/docker_mustgather.txt 2>&1
                then
                    NEW_ID=$(docker inspect -f '{{.Id}}' "$NEW_NAME" 2> /dev/null)
                    docker cp "${NEW_ID}":${IMA_SVR_DATA_PATH}/diag/cores/container_mustgather.tar.gz .
...
tar --exclude="*shadow*" --exclude="pki" -czf "$OUTPUT_FILE" $FILES_TO_COMPRESS $ADDITIONAL_DIRECTORIES --ignore-failed-read 2> /dev/null
