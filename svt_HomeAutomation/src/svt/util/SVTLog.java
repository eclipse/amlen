...
public class SVTLog {
    ...
    private static synchronized void log(STATE state, TTYPE type, String text, long n, Throwable e) {
        ...
        } else if ((SVT_Engine.t != tid)
                        || ((state == STATE.TRY) && (("client".equals(text)) || ("connection".equals(text))))) {
            if (SVT_Engine.t != 0)
                System.out.println();
            SVT_Engine.t = tid;
            if (state == STATE.TRY) {
                if (n == 0) {
                    System.out.format("%-24s %06d %-7s %6d" + ": try_" + text, time, SVTUtil.threadcount(0), type, tid);
                } else {
                    System.out.format("%-24s %06d %-7s %6d" + ": try_" + text + "#" + n, time, SVTUtil.threadcount(0),
                                    type, tid);
                }
            } else if (state == STATE.RETRY) {
                System.out.format("%-24s %06d %-7s %6d" + ": retry" + "#" + n + "_" + text, time, SVTUtil
                                .threadcount(0), type, tid);
            } else if (state == STATE.DONE) {
                if (n == 0) {
                    System.out.format("%-24s %06d %-7s %6d" + ": complete_" + text, time, SVTUtil.threadcount(0), type,
                                    tid);
                } else {
                    System.out.format("%-24s %06d %-7s %6d" + ": complete_" + text + "#" + n, time, SVTUtil
                                    .threadcount(0), type, tid);
                }
            } else if (state == STATE.DONE2) {
                if (n == 0) {
                    System.out.format("%-24s %06d %-7s %6d" + ": complete " + text, time, SVTUtil.threadcount(0), type,
                                    tid);
                } else {
                    System.out.format("%-24s %06d %-7s %6d" + ": complete " + text + "#" + n, time, SVTUtil
                                    .threadcount(0), type, tid);
                }
            }
        } else {
