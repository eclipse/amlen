...
public class ImaJson {
    ...
    public int parse(String jsonstr) {
        ...
        case StartObject:  /* Outer object is an object */
            entnum = jsonNewEnt(JObject.Object, null, null, level);
            state = JState.Name;
            break;
        case StartArray:   /* Outer object is an array */
            entnum = jsonNewEnt(JObject.Array, null, null, level);
            state = JState.Value;
            inarray = true;
            name = null;
            break;
        ...
        while (state != JState.Done) {
            ...
            case Name:
                ...
                case EndObject:
                    ...
                    if (--level >= 0) {
                        inarray = ent[where[level]].objtype == JObject.Array;
                        state = JState.Comma;
                    } else {
                        state = JState.Done;
                    }
            ...
            case Value:
                ...
                switch (token) {
                case String:
                    entnum = jsonNewEnt(JObject.String, name, value, level);
                    state = JState.Comma;
                    break;
                case Integer:
                    entnum = jsonNewEnt(JObject.Integer, name, value, level);
                    if (value.length()<10)
                        ent[entnum].count = Integer.valueOf(value);
                    else
                        ent[entnum].objtype = JObject.Number;
                    state = JState.Comma;
                    break;
                case Number:
                    entnum = jsonNewEnt(JObject.Number, name, value, level);
                    state = JState.Comma;
                    break;
                case StartObject:
                    entnum = jsonNewEnt(JObject.Object, name, value, level);
                    where[++level] = entnum;
                    inarray = false;
                    state = JState.Name;
                    break;
                case StartArray:
                    entnum = jsonNewEnt(JObject.Array, name, null, level);
                    where[++level] = entnum;
                    state = JState.Value;
                    inarray = true;
                    name = null;
                    break;
                case True:
                    entnum = jsonNewEnt(JObject.True, name, null, level);
                    state = JState.Comma;
                    break;
                case False:
                    entnum = jsonNewEnt(JObject.False, name, null, level);
                    state = JState.Comma;
                    break;
                case Null:
                    entnum = jsonNewEnt(JObject.Null, name, null, level);
                    state = JState.Comma;
                    break;
                case EndArray:
                    if (!inarray) {
     ...
     */
     public int get(int entnum, String name) {
        int maxent;

        if (entnum < 0 || entnum >= ent_count || ent[entnum].objtype != JObject.Object) {
            return -1;
        }
        ...
        while (entnum <= maxent) {
            ...
            if (ent[entnum].objtype == JObject.Object || ent[entnum].objtype == JObject.Array) {
    ...
    public int getInt(String name, int default_value) {
        ...
        switch (ent[entnum].objtype) {
        case Integer: return ent[entnum].count;
        case True:    return 1;
        case False:   return 0;
    ...
    public Object getValue(int entnum) {
        ...
        switch (ent[entnum].objtype) {
        case String:  return ent[entnum].value;
        case Integer: return ent[entnum].count;
        case Number:  return Double.parseDouble(ent[entnum].value);
        case True:    return true;
        case False:   return false;
        default:      return null;
        }
    ...
    public String [] toArray(int entnum) {
        int  maxent;
        if (entnum >= ent_count || ent[entnum].objtype != JObject.Array) {
            return null;
        }    
     ...
     */
     public boolean getBoolean(String name, boolean deflt) {
         ...
         case True:    return true;
         case False:   return false;
         case Null:    return deflt;
         default:      return deflt;
         case Integer: return ent[entnum].count != 0;
         case String:  
    ...
    public Map<String,Object> toMap(int entnum) {
        ...
        if (entnum >= ent_count || ent[entnum].objtype != JObject.Object) {
            return null;
        }    
        maxent = ent[entnum].count + entnum + 1;
        int i;
