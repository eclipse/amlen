 ...
 * 1. The number of each message type per sender (default = 3 -- 9 total msgs)
 ...
 */
public class JmsToMqttMsgConversionTest implements Runnable {
	static final int SENDER = 1;
	static final int RECEIVER = 2;
	    
	static int  count = 3; //Message of each type published per publisher
	                       //Total published = 3 * count * num publishers
...
//    	}
        ...
        for (int i=0; i<count; i++) {
            tmsg = senderSess.createTextMessage(msgtext+i);
        ...
        for (int i=0; i<count; i++) {
            smsg = senderSess.createStreamMessage();
            smsg.writeInt(i);
            smsg.writeInt(i);
            smsg.writeInt(i);
        ...
        for (int i=0; i<count; i++) {
            mmsg = senderSess.createMapMessage();
    ...
    public JmsToMqttMsgConversionTest(int insttype, int inst, IsmMqttConnection conn) {
    ...
    public JmsToMqttMsgConversionTest(int senderInst, Session session,
    ...
    public JmsToMqttMsgConversionTest() {
    ...
    public static void main(String [] args) {
    	 ...
    	 */
    	    ...
    	    String jmsportstr   = System.getenv("ISMJmsPort");
    	    String mqttportstr   = System.getenv("ISMMqttPort");
    	...
    	try {
    		...
    		for(int i=0; i < numConsumers; i++) {
        		JmsToMqttMsgConversionTest recvInst = new JmsToMqttMsgConversionTest(RECEIVER, i, recvconn[i]);
    	...
    	for(int i=1; i<numPublishers; i++) {
    		new Thread(new JmsToMqttMsgConversionTest(i, sendSess[i], prod[i])).start();
    	}
    	
    	/* Run a sender on the main thread... */
    	try {
    		JmsToMqttMsgConversionTest t = new JmsToMqttMsgConversionTest(0, sendSess[0], prod[0]);
    	    millis = t.doSend();
    	} catch (Exception ex) {
    ...
    class MqttTopicMessageListener implements IsmMqttListener {
        ...
        public void onMessage(IsmMqttMessage msg) {
            ...
            System.out.println("msg payload: "+msgstr);
