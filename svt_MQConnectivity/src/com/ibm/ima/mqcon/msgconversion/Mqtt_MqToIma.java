...
/**
 * Test sends MQTT messages to an MQ topic
 * which is received by a MQTT client listening
 * on an IMA topic.
 * 
 * It checks that the QOS is mapped correctly etc
 * 
 * Subscriber = MQTT
 * Message = MQTT
 *
 */
public class Mqtt_MqToIma extends MqttMqToImaBaseClass {
	...
	{
		Mqtt_MqToIma testClass = new Mqtt_MqToIma();
	...
	public void runTest()
	{
		...
		try
		{
			...
			if(Trace.traceOn())
			{
				Trace.trace("Sending first message: " + msg1Body + " with QOS=2");
			}
	...
	private void checkContents(MqttMessage actual)
	{
