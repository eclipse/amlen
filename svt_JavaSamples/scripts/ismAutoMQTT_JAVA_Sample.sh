...
# Test Bucket Scenarios (group 1) TO ISM as unsecure Ports
...
# Test Bucket Scenarios (group 2) TO ISM
run02Result=0
...
# Test Bucket Scenarios (group 3) TO ISM
run03Result=0
RUNCMD="../scripts/run-scenarios.sh 03.sh ${logfile}"
echo Running command: ${RUNCMD}
# removed until HA supported 
# $RUNCMD
run03Result=$?

# Test Bucket Scenarios (group 4) TO ISM
run04Result=0
...
# Test Bucket Scenarios (group 5) TO ISM
run05Result=0
...
# Test Bucket Scenarios (group 6) TO ISM
run06Result=0
...
# Test Bucket Scenarios (group 7) TO ISM
run07Result=0
...
# Test Bucket Scenarios (group 8) TO ISM
run08Result=0
RUNCMD="../scripts/run-scenarios.sh 08.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
run08Result=$?
...
exit $( expr ${run01Result} + ${run02Result} + ${run03Result} + ${run04Result} + ${run05Result} + ${run06Result} + ${run07Result} + ${run08Result})
