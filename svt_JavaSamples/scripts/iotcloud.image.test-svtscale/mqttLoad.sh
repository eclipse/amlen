...
#  use "-x option" declare -i MQTT_INSTANCES=20
#  use "-y option" declare -i MQTT_SUB_INSTANCES=20
#  use "-z & -Z option'   MQTT_ORG_INSTANCES and MQTT_INSTANCE_INDEX

ORGS_STARTED=0
DEVICES_STARTED=0
APIKEYS_STARTED=0
# Compute the Expect Sub Msgs based on number of Publisher started per ORG
let MQTT_SUB_MSGS=${MQTT_MSGS}*${MQTT_INSTANCES}
...
#   if [[ "$LOG_SUB_READY" == "TRUE" ]] ; then
      ...
      # Start the number of PUBs specifed by MQTT_INSTANCES
      while [[ ${DEVICES_STARTED} -lt ${MQTT_INSTANCES} ]] ;
      do
         let ARRAY_INDEX=${ORG_INDEX}+${DEVICES_STARTED} 
         LOG_MSG="start PUB member ${DEVICES_STARTED} in ORG ${ORGS_STARTED}, DEVICES_CLIENTID[${ARRAY_INDEX}]: ${DEVICE_CLIENTID[${ARRAY_INDEX}] } to send ${MQTT_MSGS} msgs. "
         log_msg;
...
LOG_MSG="------------------------------------------------------------------------"
log_msg;
LOG_MSG="Waiting on ${MQTT_SUB_INSTANCES} Sub and ${MQTT_INSTANCES} Pub launched in ${MQTT_ORG_INSTANCES} Orgs starting at ORG #${MQTT_INSTANCE_INDEX} to complete."
log_msg;
wait
