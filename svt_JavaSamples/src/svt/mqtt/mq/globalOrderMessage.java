...
class globalOrderMessage implements Runnable {
    ...
    private MqttSampleSubscribe gatherer = null;

    public globalOrderMessage(MqttSampleSubscribe subscriber) {
    ...
    public class sortIncoming implements Runnable {
        MqttSampleSubscribe gatherer = null;
        public List<String> sortList = Collections.synchronizedList(new LinkedList<String>());

        sortIncoming(MqttSampleSubscribe gatherer) {
            this.gatherer = gatherer;
        }

        int messagesWaitingToBeSorted() {
            return gatherer.receivedMessageList.size();
        }
        ...
        private void sort() {
            mutex.acquireUninterruptibly();
            size = Math.min(gatherer.receivedMessageList.size(), 2000);
            for (int i = 0; i < size; i++) {
                sortCount(1);
                sortBySubscriber(sortedBySubscriber, gatherer.receivedMessageList.remove(0));
            }
            mutex.release();
        }
