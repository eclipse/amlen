...
for qos in 0 1 2 ; do 
    while (($i<$num_clients)); do
        echo "./mqttsample_array -a subscribe -i ${qos}_${i}_${now}_S `get_flags $qos` -n 0 -c true&" |sh -x
        let i=$i+1;
    done
    echo "./mqttsample_array -a publish -i ${qos}_${now}_P `get_flags $qos` -n 0 -c true &" |sh -x
done
