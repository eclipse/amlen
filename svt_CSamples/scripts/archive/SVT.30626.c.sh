...
for qos in $qos_under_test ; do 
    ...
    while (($i<$num_clients)); do
        ...
        #-----------------------------------
        #-----------------------------------
        # 4/17/13: Note:added back in -r option
        #-----------------------------------
        echo "MQTT_C_CLIENT_TRACE=ON ./mqttsample_array -o log.p.${name}.${qos}.${i}.${now} -r log.p.${name}.${qos}.${i}.${now}.persist -a publish -i ${qos}_${i}_${now}_P `get_flags $qos PUBLISH` &" |sh -x> log.p.${name}.${qos}.${i}.${now}.mqtt
        let i=$i+1;
    done
...
echo ""
echo "------------------------------------------------"
echo "`date` DETAILED Publisher logs"
find log.p.${name}.${qos}.*${now} | xargs -i echo "echo ================================ ; echo Details for {} ; echo ================================ ; cat {} | awk '/SVT_MQTT_C_INFO_START/,/*/ {print \$0}' "   |sh
echo ""
echo "------------------------------------------------"
echo ""
