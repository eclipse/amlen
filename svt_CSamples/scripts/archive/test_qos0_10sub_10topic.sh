...
rm -rf ./out.RESULT.10.subscribers.10.topics

subscriber_clients=10;

let rate=50000;
while (($rate<800000)); do
    rm -rf log.*
...
find out.qos.* | xargs -i echo " grep \"^Start test for rate \" {} | awk '{printf(\"%s \", \$5);}' ; cat {} | grep ^"./log.s" | grep actual_msgs | awk '{printf(\"%s \", \$8); }' ; cat {} | grep ^"./log.p" | grep actual_rate | awk '{printf(\"%s \", \$8); }' ; echo COMPLETE " |sh   | sort -n >> ./out.RESULT.10.subscribers.10.topics
