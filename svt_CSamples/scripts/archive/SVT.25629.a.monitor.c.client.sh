...
#for file_pattern in $INPUT_FILE_PATTERN ;  do
    ...
    echo -n "1" >> /dev/stderr
    data=`echo "$files" | xargs -i  echo "grep actual_ {} " | sh `
    rc=$?;
    echo -n "." >> /dev/stderr
    if [ -n "$data" ] ; then
        result_available=1;
    ...
    echo -n "2" >> /dev/stderr
    data=`echo "$files" | xargs -i  echo "grep -hoE 'MQTTClient_connect rc.*' {} " | sh `
    rc=$?;
    echo -n "." >> /dev/stderr 
    ...
    if [ -n "$data" ] ; then
        echo -n "$file_pattern Connection Time :$result_available:"
        echo "$data" | awk '{print $5}'   | print_latest_stats ;
    echo -n "." >> /dev/stderr
        echo -n "$file_pattern Connection rc   :$result_available:"
        echo "$data" | awk '{print $3}'   | print_latest_stats ;
    echo -n "." >> /dev/stderr
        echo -n "$file_pattern Connection count:$result_available:"
        echo "$data" | awk '{print $7}'   | print_latest_stats ;
    echo -n "." >> /dev/stderr
        echo -n "$file_pattern Connection fails:$result_available:"
        echo "$data" | awk '{print $9}'   | print_latest_stats ;
    echo -n "." >> /dev/stderr
    else
        echo    "$file_pattern                 :$result_available: no data - connections : rc: $rc data: $data"; 
    fi 

    echo -n "3" >> /dev/stderr
    data=`echo "$files" | xargs -i  echo "tac {} | grep SVT_MQTT_C_STATUS | head -1" |sh `
    rc=$?;
    echo -n "." >> /dev/stderr
