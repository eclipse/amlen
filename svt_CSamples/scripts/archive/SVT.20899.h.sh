...
get_flags(){
    local qos=$1
    local action=$2
    local flags=" -s ${ima1} -x haURI=${ima2} -x keepAliveInterval=60 -x connectTimeout=10 -x reconnectWait=1 -x retryConnect=3600 -q ${qos} -t ${qos}_${now} -x verifyMsg=1 -x orderMsg=1 -x msgFile=./SMALLFILE -x statusUpdateInterval=1 -x subscribeOnConnect=1 -x publishDelayOnConnect=120  "
...
for qos in $qos_under_test ; do 
    ...
    while (($i<$num_clients)); do
        echo "MQTT_C_CLIENT_TRACE=ON ./mqttsample_array -o log.s.${qos}.${i}.${now} -r log.s.${qos}.${i}.${now}.persist -a subscribe -i ${qos}_${i}_${now}_S `get_flags $qos SUBSCRIBE` &" |sh -x > log.s.${qos}.${i}.${now}.mqtt 
        let i=$i+1;
    done
done
...
for qos in $qos_under_test ; do 
    #rm -rf log.p.${qos}.*
    let i=0;
    echo "MQTT_C_CLIENT_TRACE=ON ./mqttsample_array -o log.p.${qos}.${now} -a publish -r log.p.${qos}.${now}.persist -i ${qos}_${now}_P `get_flags $qos PUBLISH` &" |sh -x> log.p.${qos}.${now}.mqtt
