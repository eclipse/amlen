...
declare -a test_array=(
    ...
    #---------------------------------------------------------
    SVT_01a 1800 
        "SVT_AT_VARIATION_QOS=0|SVT_AT_VARIATION_MSGCNT=3|SVT_AT_VARIATION_PUBRATE=0.05|SVT_AT_VARIATION_SYNC_CLIENTS=true" 
    ...
    #---------------------------------------------------------
    SVT_01b 1800
        "SVT_AT_VARIATION_QOS=1|SVT_AT_VARIATION_MSGCNT=3|SVT_AT_VARIATION_PUBRATE=0.05|SVT_AT_VARIATION_SYNC_CLIENTS=true|SVT_AT_VARIATION_CLEANSESSION=false" 
        "Scale MQTTv3 connection workload qos 1 clean false"

    #---------------------------------------------------------
    # Test bucket SVT_01b: variation - same as Test bucket 1 , but now with qos 2 and publishing every 20 seconds
    #---------------------------------------------------------
    SVT_01c 1800 
        "SVT_AT_VARIATION_QOS=2|SVT_AT_VARIATION_MSGCNT=3|SVT_AT_VARIATION_PUBRATE=0.05|SVT_AT_VARIATION_SYNC_CLIENTS=true|SVT_AT_VARIATION_CLEANSESSION=false"
        "Scale MQTTv3 connection workload qos 2 clean false"
    #---------------------------------------------------------
    # Test bucket SVT_01c: variation - same as Test bucket 1 , but now with qos 0, 100 msgs/client, in bursts of 10 every 20 sec
    #---------------------------------------------------------
    SVT_01d 3600 
        "SVT_AT_VARIATION_QOS=0|SVT_AT_VARIATION_MSGCNT=20|SVT_AT_VARIATION_PUBRATE=0.05|SVT_AT_VARIATION_SYNC_CLIENTS=true|SVT_AT_VARIATION_BURST=10" 
        "Scale MQTTv3 connection workload"
...
#    echo "------------3--------------"
    ...
    while [ "${test_array[${j}]}" != "NULL" ] ; do
        ...
        if [[ $a_variation =~ $regexp ]] ; then
            ...
            regexp="SVT_AT_VARIATION_TEST_ONLY=true"
            if ! [[ $a_variation =~ $regexp ]] ; then
                test_template_add_test_single  "${a_name} - performing automatic cleanup for durable subscriptions from previous variation. "  cAppDriver m1 "${a_command}" "${a_timeout}" "${a_variation}|SVT_AT_VARIATION_ALL_M=true|SVT_AT_VARIATION_MSGCNT=0|SVT_AT_VARIATION_CLEANSESSION=true|SVT_AT_VARIATION_SYNC_CLIENTS=false|SVT_AT_VARIATION_SKIP_PUBLISH=1" "TEST_RESULT: SUCCESS"
            fi
...
do_setup() {
    ...
    local admin=${2-"${A1_HOST}"}

    #---------------------------------------------------------
    # One time setup for all subsequent tests. (Virtual Nic setup on server)
    #---------------------------------------------------------
    test_template_add_test_single "Scale MQTTv3 connection workload - setup server for ${appliance}" cAppDriver m1 "-e|${M1_TESTROOT}/svt_cmqtt/SVT.start.workload.sh,-o|connections_setup|SERVER|1|${appliance}" 600 "SVT_ADMIN_IP=${admin}" "TEST_RESULT: SUCCESS"

    #---------------------------------------------------------
    # One time setup for all subsequent tests. (sysctl, Virtual Nic setup on client)
    #---------------------------------------------------------

    test_template_add_test_single "Scale MQTTv3 connection workload - setup client for ${appliance}" cAppDriver m1  "-e|${M1_TESTROOT}/svt_cmqtt/SVT.start.workload.sh,-o|connections_setup|CLIENT|1|${appliance}" 600 "SVT_AT_VARIATION_ALL_M=true|SVT_AT_VARIATION_QUICK_EXIT=true|SVT_ADMIN_IP=${admin}" "TEST_RESULT: SUCCESS"
...
do_ha_main_launch() {
    ...
    num_clients=100000 ; # less workload for final HA tests to make sure things run smoothly on client side.
...
do_ha_tests() {
	...
	if (($A_COUNT>1)) ; then # don't try to do HA testing unless A_COUNT reports at least 2 appliances.
    	...
    	do_setup ${A2_IPv4_1} ${A2_HOST} # make sure backup is setup too.
...
do_standalone_tests() {
    ...
    launch_fill_drain_tests 11000 "SVT_PF_CRITERIA_ORDERING=log.*"    # New - added for RTC 43580
    ...
    num_clients=249500; # try to fix RTC 35918 with less clients
    ...
    do_client_cert_with_unique_CommonName_for_each_client 100000 ; # 100K clients with unique CommonName
}

svt_do_automatic_backup # only operates in sandbox environment to prevent accidental file deletes


do_init
do_setup ${A1_IPv4_1} ${A1_HOST}
#do_sandbox_launch
do_standalone_tests
do_ha_tests
