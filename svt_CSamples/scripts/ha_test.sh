...
ima1_discovery_interface=ha0
ima2_discovery_interface=ha0
...
set_environment () {
    ...
    if echo "$ima_status" | grep "Running" >/dev/null ;then
    ...
    elif echo "$ima_status" | grep "Standby" > /dev/null ;then
...
get_status () {
    ...
    while((1)) ; do
        ...
        msg+="$h1"
        msg+=$'\nstatus of ima1 is : '
        status1=`ssh_wrapper -nx  admin@$ima1 status imaserver `
        s1=`echo "$status1" | grep "Status = "`
        msg+="$s1"
        msg+=$'\n'
        ...
        if [[ "$s1" =~ "$needed_status_2" ]] ;then
            n2=1
        fi

        if [ "$needed_status_1" != "" -a "$n1" == "1" ] ;then
            msg+="needed_status_1 is met"
            msg+=$'\n'
            ((total_status_1++));
        elif [ "$needed_status_2" != "" -a "$n2" == "1" ] ;then
        ...
        set_environment "IMA1" $ima1 `echo "$s1" | grep ^Status | head -1 | awk '{printf("%s%s", $3,$4);}'` `echo "$h1" | grep ^NewRole | head -1 | awk '{printf("%s", $3);}'`
        ...
        msg+="$h2"
        msg+=$'\nstatus of ima2 is : '
        status2=`ssh_wrapper -nx  admin@$ima2 status imaserver `
        s2=`echo "$status2" | grep "Status = "`
        msg+="$s2"
        msg+=$'\n'
        msg+="needed_status_1 is $needed_status_1"
        msg+=$'\n'
        ...
        if [[ "$s2" =~ "$needed_status_2" ]] ;then
            n2=1
        fi

        if [ "$needed_status_2" != "" -a "$n2" == "1" ] ;then
            msg+="needed_status_2 is met"
            msg+=$'\n'
            ((total_status_2++));
        elif [ "$needed_status_1" != "" -a "$n1" == "1" ] ;then
            msg+="needed_status_1 is met"
            msg+=$'\n'
            ((total_status_1++));
        else
        ...
        set_environment "IMA2" $ima2 `echo "$s2" | grep ^Status | head -1 | awk '{printf("%s%s", $3,$4);}'` `echo "$h2" | grep ^NewRole | head -1 | awk '{printf("%s", $3);}'`
...
ha_pair_up_old() {
    ...
    if run_command_with_timeout "get_status \"Running (production)\" 1 "Standby" 1 5 " 10 ; then
    ...
    start_servers
    let rc=$rc+$?;
    get_status "Running (production)" 1 "Standby" 1
    let rc=$rc+$?;
    return $rc;
}
#----------------------------------------
...
ha_pair_up_part1() {
    ...
    if run_command_with_timeout "get_status \"Running (production)\" 1 "Standby" 1 60 " 120 ; then
        echo "HA Pair is already up at Running and Standby"
        return 0;
    else
        echo "HA pair - force into known good state"
...
ha_pair_up_part2b() {
    ...
    date
    echo "=================================="
    echo "set maintenance mode on both"
    echo "=================================="
    ssh_wrapper -nx  admin@$ima1 "imaserver runmode maintenance"
    ssh_wrapper -nx  admin@$ima2 "imaserver runmode maintenance"
    while((1)); do
        kill_servers
        let rc=$rc+$?;
        if ! get_status "Stopped" 2 "" 0 300 ; then
            echo "Still waiting for both of the servers to go into Stopped"
        else
            break;
        fi
...
ha_pair_up_part3b_helper(){
    local ima=$1
    local flag=${2-"PRIMARY"} # STANDBY or PRIMARY
    ...
    echo "`date` step b..."
    echo "=================================="
    get_status "Running (maintenance)" 1 "Stopped" 1
    echo "=================================="
    echo "`date` step c..."
    echo "=================================="
    if [ "$flag" == "PRIMARY" ] ;then
        ssh_wrapper -nx  admin@$ima "imaserver update HighAvailability StartupMode=StandAlone EnableHA=True "
    else
        ssh_wrapper -nx  admin@$ima "imaserver update HighAvailability StartupMode=AutoDetect EnableHA=True "
    fi
    ssh_wrapper -nx  admin@$ima "imaserver runmode clean_store"
    echo "=================================="
    echo "`date` step d..."
    echo "=================================="
    stop_server $ima
    if ! get_status "Stopped" 1 "Stopped" 1 60 ; then
        echo "=================================="
        echo "`date` step d1..."
        echo "=================================="
        kill_server $ima 
        ...
        echo "`date` step d2..."
        echo "=================================="
        get_status "Stopped" 1 "Stopped" 1
    fi
    echo "=================================="
    echo "`date` step e..."
    echo "=================================="
    ...
    echo "`date` step g..."
    echo "=================================="
    ssh_wrapper -nx  admin@$ima "imaserver runmode production"
    echo "=================================="
    echo "`date` step h..."
    echo "=================================="
    stop_server $ima
    ...
    if [ "$flag" == "PRIMARY" ] ;then
        ...
        echo "`date` step j..."
        echo "=================================="
        get_status "Running (production)" 1 "Stopped" 1
        ssh_wrapper -nx  admin@$ima "imaserver update HighAvailability StartupMode=AutoDetect EnableHA=True "
        echo "=================================="
        echo "`date` step k..."
        echo "=================================="
...
ha_pair_set_timezone(){
    ssh_wrapper -nx  admin@$ima1 "timezone set CST6CDT"
    ssh_wrapper -nx  admin@$ima2 "timezone set CST6CDT"
}
ha_pair_set_ntp_time(){
    ssh_wrapper -nx  admin@$ima1 "set-ntp-servers 10.10.10.10"
    ssh_wrapper -nx  admin@$ima2 "set-ntp-servers 10.10.10.10"
}

ha_pair_up_part3b() {
    echo "=================================="
    echo "`date` Set timezone"
    echo "=================================="
    ha_pair_set_timezone
    echo "=================================="
    echo "`date` Set NTP servers"
    echo "=================================="
    ha_pair_set_ntp_time
    ...
    ha_pair_up_part3b_helper $ima1 "PRIMARY"
    ha_pair_up_part3b_helper $ima2 "STANDBY"
    start_server $ima1
    start_server $ima2
    echo "=================================="
    echo "`date` Verify Running / Standby status"
    echo "=================================="
    get_status "Running (production)" 1 "Standby" 1

    echo "=================================="
    echo "TODO : MAke sure ntp time is setup"
    echo "TODO : MAke sure ntp time is setup"
    echo "TODO : MAke sure ntp time is setup"
    echo "  TODO    10.23.2013    TODO      "
    echo "TODO : MAke sure ntp time is setup"
    echo "TODO : MAke sure ntp time is setup"
    echo "TODO : MAke sure ntp time is setup"
    echo "=================================="
...
ha_pair_up_part3() {
    ...
    echo "=================================="
    echo "`date` Set clean_store on $ima2 and stop server, verify both stopped."
    echo "=================================="
    ssh_wrapper -nx  admin@$ima1 "imaserver runmode clean_store"
    stop_server $ima2
    ...
    echo "=================================="
    echo "`date` Start servers in maintenance to set production"
    echo "=================================="
    start_servers
    #start_server $ima2
    #get_status "Stopped" 1 "Running (maintenance)" 1
    echo "=================================="
    echo "`date` Set production and stop"
    echo "=================================="
    ssh_wrapper -nx  admin@$ima1 "imaserver runmode production"
    ssh_wrapper -nx  admin@$ima2 "imaserver runmode production"
    stop_server $ima1
    stop_server $ima2
    echo "=================================="
    echo "`date` Verify stopped status"
    echo "=================================="
    get_status "Stopped" 2
    echo "=================================="
    echo "`date` Start back up"
    echo "=================================="
    start_server $ima1
    start_server $ima2
    echo "=================================="
...
ha_pair_cleanup_b() {
    ...
    echo "`date` stop mqconnectivity" 
    ssh_wrapper -nx  admin@$ima1 "imaserver stop mqconnectivity"
    ssh_wrapper -nx  admin@$ima2 "imaserver stop mqconnectivity"
    ha_pair_up_part2b;
    ha_pair_up_part3b;
    echo "`date` start mqconnectivity" 
    ssh_wrapper -nx  admin@$ima1 "imaserver start mqconnectivity"
    ssh_wrapper -nx  admin@$ima2 "imaserver start mqconnectivity"
    
}

get_primary(){
    get_status "Running (production)" 1  > /dev/null
    if [ "$?" != "0" ] ;then
        return 1; 
    fi
    echo $IMA_HA_PRIMARY
...
svt_ha_test_print_current_status () {
    local server
    local data

    for server in $ima1 $ima2 ; do 
        echo "-------------------------------------------------------------"
        echo "`date` svt_ha_test_print_current_status START : $server"
        echo "-------------------------------------------------------------"
        ssh -nx  admin@$server " echo =====================   status imaserver ====================== ;  status imaserver; echo =====================   datetime get ====================== ;  datetime get; echo =====================   imaserver harole ====================== ;  imaserver harole; " 
        echo "-------------------------------------------------------------"
        echo "`date` svt_ha_test_print_current_status END   : $server"
        echo "-------------------------------------------------------------"
    done
...
#
# Procedure:
# 1. HA pair up and running in "Running (production)" "Standby"
...
# 3. Expect status to reach "Stopped" and "Running (production)"
# 4. Start imaserver again on new standby
# 5. Expect status to return "Standby" and "Running (production)"
...
svt_ha_test_X_disrupt_server_on_Y(){
    ...
    if [ "$method" == "KILL" ] ;then
        ...
        echo "---------------------------"
        get_status "Running (production)" 2
        let ret=$ret+$?
        ...
        echo "---------------------------"
        if ! get_status "Running (production)" 1 "Maintenance" 1  60 ; then
            get_status "Running (production)" 1 "Standby" 1 
...
svt_ha_setup_20_91(){
    echo export A1_IPv4_HA0=10.11.1.20 
    echo export A1_IPv6_HA0=fc00::10:11:1:20  
    echo export A1_IPv4_HA1=10.12.1.20 
    echo export A1_IPv6_HA1=fc00::10:12:1:20  
    echo export A2_IPv4_HA0=10.11.1.91 
    echo export A2_IPv6_HA0=fc00::10:11:1:91
    echo export A2_IPv4_HA1=10.12.1.91 
    echo export A2_IPv6_HA1=fc00::10:12:1:91
}


svt_ha_setup_125_126(){
    local IMA=10.10.10.10
    local IMAe="1.125"

    echo ""
    echo "Add these to ISMsetup.sh"
    echo ""
    echo export A1_IPv4_HA0=10.11.1.125 
    echo export A1_IPv6_HA0=fc00::10:11:1:125  
    echo export A1_IPv4_HA1=10.12.1.125 
    echo export A1_IPv6_HA1=fc00::10:12:1:125  
    echo export A2_IPv4_HA0=10.11.1.126 
    echo export A2_IPv6_HA0=fc00::10:11:1:126
    echo export A2_IPv4_HA1=10.12.1.126 
    echo export A2_IPv6_HA1=fc00::10:12:1:126
    echo ""
    echo ""
    echo export A1_IMMIP=10.10.10.10
    echo export A2_IMMIP=10.10.10.10
    echo ""
    
    echo "ssh admin@$IMA \"edit ethernet-interface ha0; ip ; address 10.11.${IMAe}/23; exit ; exit; enable ethernet-interface ha0;\""
    echo "ssh admin@$IMA \"edit ethernet-interface ha1; ip ; address 10.12.${IMAe}/23; exit ; exit; enable ethernet-interface ha1;\" "
    
    echo "ssh admin@$IMA \"edit ethernet-interface eth6; ip ; address 10.10.1.125/23; exit ; exit; enable ethernet-interface eth6;\" "
    IMA=10.10.10.10
    IMAe="1.126"

    echo "ssh admin@$IMA \"edit ethernet-interface ha0; ip ; address 10.11.${IMAe}/23; exit ; exit; enable ethernet-interface ha0;\""
    echo "ssh admin@$IMA \"edit ethernet-interface ha1; ip ; address 10.12.${IMAe}/23; exit ; exit; enable ethernet-interface ha1;\""
    echo "ssh admin@$IMA \"edit ethernet-interface eth6; ip ; address 10.10.1.126/23; exit ; exit; enable ethernet-interface eth6;\" "

    echo ""
    echo "If this is the first time configuring HA , then also you need to run this: "
    echo "/niagara/test/scripts/haFunctions.sh -a setupHA -f /dev/stdout"
    echo ""
}

svt_ha_power_status_125_126 () {
    ipmitool -I lanplus -H 10.10.10.10 -U USERID -P PASSW0RD power status
    ipmitool -I lanplus -H 10.10.10.10 -U USERID -P PASSW0RD power status
