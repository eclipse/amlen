...
void do_info_display(int flag) {
    ...
    r->elapsed = (double)(r->currTime - r->firstMsgTime);
    ...
    if(flag == MYBASIC){
        LOG_STDOUT("SVT_MQTT_C_STATUS,%f,%llu,%s,(actuals for rate,msgs,lastmsgtime)\n" ,
            (double)r->nmsgs/((double)(r->currTime-r->firstMsgTime)/MICROS_PER_SECOND),
            r->nmsgs,
        ...
        LOG_STDOUT("SVT_MQTT_C_INFO_START\n" \
            ...
            "SVT_MQTT_C_INFO_END\n",
            r->desiredMsgsPerSec, 
            (double)r->nmsgs/((double)(r->lastMsgTime-r->firstMsgTime)/MICROS_PER_SECOND),
            r->nmsgs, ((double)(r->nmsgs)/(double)g_msg_number)*100.0, g_msg_number,
...
void do_status_update(void){
    ...
    if(g_verifyStillActive>0){
        if( g_rate_s.nmsgs > g_lastActiveCount ){
            g_lastActiveCount=g_rate_s.nmsgs; 
            g_lastActiveTime=currTime;
        } else if (((currTime-g_lastActiveTime) > g_verifyStillActive ) && (g_lastActiveCount > 0)){
            LOG_ERROR("Verification failed for verifyStillActive setting expired : %llu. " \
                " halting program execution due to inactivity\n",
                g_verifyStillActive);
            g_halt++;
        }
    }
...
void do_rate_control(rateControl *r){
    ...
    if (r==NULL) {
        ...
        if(g_rate_s.firstMsgTime == 0){
            g_rate_s.firstMsgTime=getHRTimer();
        }
...
if(g_trace_level>0){
    ...
    printf("         valid keys are: verifyStillActive|\n");
if(g_trace_level>0){
    printf("     verifyStillActive :  \n                       : ");
    printf(" If you want the client to verify that it is still active set this value to a\n" \
...
void do_parse_arguments(int argc , char **argv){
    ...
    while ((c=getopt(argc,argv,"a:c:hi:m:n:o:p:q:r:s:S:t:u:vw:x:")) != -1) {
        switch (c) {
            case 'a':
            ...
            case 'm':
                ...
                break;
            case 'n':
                g_msg_number=strtoull(optarg,0,10);
                LOG_INFO( "g_msg_number is %llu\n", g_msg_number);
                break;
            case 'o':
            ...
            case 'w':
                ...
                break;
            case 'x':
                g_verifyStillActive=strtoull(optarg+18,0,10)*MICROS_PER_SECOND;
                if (g_verifyStillActive < 0){
