...
{
	printf("Syntax:\n\n");
	printf("    MQTTV3ASSample [-h] [-a publish|subscribe] [-t <topic>] [-m <message text>]\n");
	...
	printf("            (publish: \"MQTTV3ASSample/C/v3\", subscribe: \"MQTTV3ASSample/#\")\n");
...
int main(int argc, char** argv)
{
	...
	if (options.topic == NULL || ( options.topic != NULL && strlen(options.topic) == 0) )
	{
		...
		if (strcmp(options.action, "publish") == 0)
			options.topic = "MQTTV3ASSample/C/v3";
		else
			options.topic = "MQTTV3ASSample/#";
	}
