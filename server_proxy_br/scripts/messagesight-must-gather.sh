...
IS_DOCKER=0
ADDITIONAL_DIRECTORIES="/etc /var/log ${IMA_BRIDGE_DATA_PATH}"
...
# 3. Locate imaserver directory
    ...
    then
        IMASERVER_SOURCE=${IMA_BRIDGE_INSTALL_PATH}
    fi
...
then
    ...
    then
        ...
        docker exec -it "$CONTAINER_ID" ${IMA_BRIDGE_INSTALL_PATH}/bin/internal_mustgather.sh > "${WORK_DIR}"/docker_mustgather.txt 2>&1

        docker cp "${CONTAINER_ID}":${IMA_BRIDGE_DATA_PATH}/diag/cores/container_mustgather.tar.gz . 2> /dev/null
    else
        ...
        then
            ...
            docker run "$ADDITIONAL_PARMS" --volumes-from "$CONTAINER_ID" --entrypoint=${IMA_BRIDGE_INSTALL_PATH}/bin/internal_mustgather.sh --name "$NEW_NAME" "$DEBUG_IMAGE_NAME" > "${WORK_DIR}"/docker_mustgather.txt 2>&1
            if [ $? -eq 0 ]
            then
                NEW_ID=$(docker inspect -f '{{.Id}}' "$NEW_NAME" 2> /dev/null)
                docker cp "${NEW_ID}":${IMA_BRIDGE_DATA_PATH}/diag/cores/container_mustgather.tar.gz .
...
tar --exclude="*shadow*" --exclude="pki" --exclude="${IMA_BRIDGE_DATA_PATH}/keystore" -czf "$OUTPUT_FILE" $FILES_TO_COMPRESS $ADDITIONAL_DIRECTORIES --ignore-failed-read 2> /dev/null
