...
if [ -z $INSTALLDIR ]
then
        INSTALLDIR=${IMA_BRIDGE_INSTALL_PATH}
fi
export INSTALLDIR
...
echo "----------------------------------------------------------------" >> $OUTF
echo "" >> $OUTF
echo "`strings ${IMA_BRIDGE_INSTALL_PATH}/bin/imabridge | grep version | grep IBM`" >> $OUTF
...
do
    ...
    if [ $? -eq 0 ]
    then
        PROCBIN="${IMA_BRIDGE_INSTALL_PATH}/bin/imabridge"
    fi
