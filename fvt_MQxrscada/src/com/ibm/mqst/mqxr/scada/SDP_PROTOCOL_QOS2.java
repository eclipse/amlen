...
public class SDP_PROTOCOL_QOS2
{
    ...
    public static void main(String[] args)
    {
        ...
        PUBLISHmsg pubOut11; // QoS 2 - PUBLISH with DUP
        ...
        PUBLISHmsg pubIn11;
        PUBLISHmsg pubIn11EX; // Qos 2 - PUBLISH with DUP
        ...
        MsgIdentifier pub11Id; // QoS 2 - publish DUP
        ...
        byte[] payload11 = "QoS2 - Only send DUP Message".getBytes();
        byte subQos = 2;
        byte pub2Qos = 1;
        ...
        try
        {
            ...
            publishConnectMessage = new CONNECTmsg(publishClientId,true);
            resultsLog.println("Message created in " + connectLog.name);
            ...
            try
            {
                ...
                reconnectAck = new CONNACKmsg();
               	reconnectAck.setSessionPresent(true);
                subscribeConnectMessage.sendAndAcknowledge(subscribeInput, subscribeOutput, reconnectAck, connectLog, resultsLog);
...
/*
            ...
            try
            {
                ...
                pubCompEX = new PUBCOMPmsg(pubOut8.myId, 146);
            ...
            try
            {
                ...
                reconnectAck = new CONNACKmsg();
                reconnectAck.setSessionPresent(true);
                subscribeConnectMessage.sendAndAcknowledge(subscribeInput, subscribeOutput
                		, reconnectAck, connectLog, resultsLog);
            ...
            /*
             * We need to check QoS2 msg flow behaves as described in the SCADA
             * spec, if we send a PUBLISH with DUP because server never received -
             * the original (we actually don't send).
             */
            try
            {
                // Create publish data and publish
                pub11Id = msgIds.getId();
    
                resultsLog.printTitle("TESTING PUBLISH MESSAGES (Qos2) - no PUBCOMP sent on get");
                resultsLog.println("Creating publication message - QoS 2");
    
                pubOut11 = new PUBLISHmsg(topic, pub11Id, payload11, pub11Qos);
                pubOut11.setDUP();
    
                resultsLog.println("PUBLISH message created in " + pubLog.name);
                resultsLog.println("Sending PUBLISH message to broker...");
    
                pubOut11.sendAndAcknowledge(publishInput, publishOutput, pubLog, resultsLog);
    
                // Should now get the PUBLISH msg back - we are subscribed
    
                resultsLog.println("Waiting for published data");
    
                pubIn11EX = new PUBLISHmsg(topic, pub11Id, payload11, pub11Qos);
                pubIn11 = new PUBLISHmsg(subscribeInput);
                pubIn11EX.setId(pubIn11.myId);
    
                SCADAutils.compareAndLog(pubIn11, pubIn11EX, pubLog, resultsLog);
                resultsLog.println("Sending PUBREC...");
    
                pubRec = new PUBRECmsg(pubIn11.myId);
                ...
                resultsLog.println("Waiting for PUBREL reply...");
                pubRelEX = new PUBRELmsg(pubIn11.myId);
                pubRel = new PUBRELmsg(subscribeInput);
    
                SCADAutils.compareAndLog(pubRel, pubRelEX, pubLog, resultsLog);
                resultsLog.println("PUBREL received - sending final PUBCOMP...");
    
                pubComp = new PUBCOMPmsg(pubIn11.myId);
            ...
            // Need to clean up the non-clean session
            ...
            subscribeOutput = new DataOutputStream(subscribeSocket.getOutputStream());
            subscribeInput = new DataInputStream(subscribeSocket.getInputStream());
            subscribeConnectMessage = new CONNECTmsg(subscribeClientId, true);
            subscribeConnectMessage.sendAndAcknowledge(subscribeInput, subscribeOutput
