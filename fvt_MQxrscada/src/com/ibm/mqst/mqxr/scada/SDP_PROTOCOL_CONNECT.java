...
public class SDP_PROTOCOL_CONNECT
{
    ...
    {
        // Testcase
        String testName = "SDP_PROTOCOL_CONNECT";
        ...
        try
        {
            ...
            /** *********************************************************** */
            // Connect to broker with CleanSession true
            /** *********************************************************** */
            ...
            try
            {
                ...
                resultsLog.printTitle("SUBSCRIBE WITH QOS VALUE 2");
                resultsLog.println("Subscribing to " + topic1);
          
                subList = new SubList(topic1, qos2);
                subId = msgIds.getId();
                ...
                sub.sendAndAcknowledge(subscriber.getInputStream(), subscriber.getOutputStream(), connectLog, resultsLog);
            ...
            /** *********************************************************** */
            // Connect with cleanStart = true
            /** *********************************************************** */

            try
            {

                resultsLog.printTitle("Connect with cleanSession false, subscribe, disconnect reconnect cleanSession=false");
                msg = new CONNECTmsg(subscriberClientId, false, SCADAutils.protocolName, SCADAutils.version);
                msg.sendAndAcknowledge(subscriber.getInputStream(), subscriber.getOutputStream(), connectLog, resultsLog);
                resultsLog.println("Subscribing to " + topic1);
                ...
                // Connect to broker and wait for reply
                resultsLog.printTitle("RECONNECTING SUBSCRIBER CLIENT TO BROKER 2");
                msg = new CONNECTmsg(subscriberClientId, false, SCADAutils.protocolName, SCADAutils.version);
                connackEx = new CONNACKmsg();
               	connackEx.setSessionPresent(true);
                msg.sendAndAcknowledge(subscriber.getInputStream(), subscriber.getOutputStream()
                		, connackEx, connectLog, resultsLog);
                ...
                try
                {
                    pubAck = new PUBACKmsg(publisher.getInputStream());
                    if (SCADAutils.version >= 5) {
                        pubAck.parseAndPrint(connectLog);
                        /* TODO: test */
                    } else {
            ...
            // Connect to broker and wait for reply
            resultsLog.printTitle("RECONNECTING PUBLISHER CLIENT TO BROKER 3");
            ...
            try
            {
            	CONNACKmsg connackEx2 = new CONNACKmsg(UNACCEPTABLE_PROTOCOL_VERSION);
            ...
            try
            {
            	CONNACKmsg connackEx2;
            	if (SCADAutils.getVersion() >= 5)
            	    connackEx2 = new CONNACKmsg(INVALID_VERSION_5);
            	else    
            	    connackEx2 = new CONNACKmsg(UNACCEPTABLE_PROTOCOL_VERSION);
                msg.sendAndAcknowledge(invalidClient.getInputStream(), invalidClient.getOutputStream(), connackEx2, connectLog, resultsLog);
