...
public class PUBLISHmsg extends MsgTemplate
{
    ...
    public PUBLISHmsg(DataInputStream in, byte header) throws IOException
    {
    ...
    void setNoSubscription(boolean nosub) {
    ...
    private void buildMsg() throws ScadaException
    {
        ...
        if (SCADAutils.version >= 5) {
            for (int i = 0; i < props.length; i++) {
                construct.addElement(new Byte(props[i]));
            }
        }
    ...
    protected void setFields()
    {
        ...
        if (SCADAutils.version >= 5) {
            int proplen = SCADAutils.remainingLength(completeMsg,  offset);
            if (proplen > 127)
                proplen++;
            proplen++;
            props = new byte[proplen];
            for (int i=0; offset < props.length; offset++, i++) {
                props[i] = completeMsg[offset];
            }    
        }
    ...
    public void sendAndAcknowledge(DataInputStream input, DataOutputStream output, LogFile pubLog, LogFile resultsLog) throws IOException, ScadaException
    {
        ...
        if (myQos == 1)
        {
            ...
            PUBACKmsg pubAckEX = new PUBACKmsg(myId, noSubscription ? 16 : 0);
            if (SCADAutils.version >= 5) {
                if (pubAck.fixedHeader == (byte)0xE0) {
                    throw new SocketException("DISCONNECT control packet received");
                }
            }
        ...
        else if (myQos == 2)
        {
            ...
            PUBRECmsg pubRecEX = new PUBRECmsg(myId, noSubscription ? 16 : 0);
            if (SCADAutils.version >= 5) {
                if (pubRec.fixedHeader == (byte)0xE0) {
                    throw new SocketException("DISCONNECT control packet received");
                }
            }
    	...
    	if (null != realMsg) {
            ...
            if (SCADAutils.version >= 5) {
                ...
                for (int i=0; i < proplen; i++) {
                    SCADAutils.printByteNumeric(completeMsg[offset], log, "PROPERITES", offset);
                    offset++;
                }    
            }
