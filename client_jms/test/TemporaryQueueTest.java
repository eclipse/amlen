 ...
 */
public class TemporaryQueueTest implements Runnable {
    int action;
    ...
    static TemporaryQueue tempResponseQ;
    static HashMap<Queue, MessageProducer> replytoProds = new HashMap<Queue, MessageProducer>();
     ...
     */
    public TemporaryQueueTest(int action) {
        this.action = action;
    }
    ...
    public static long doSend() throws JMSException {
        ...
        tempResponseQ = sess.createTemporaryQueue();
        MessageProducer prod = sess.createProducer(requestQ);
        prod.setDeliveryMode(DeliveryMode.PERSISTENT);
        new Thread(new TemporaryQueueTest(RECV)).start();
        conn.start();
        int commitcount = 0;
        try {
            Thread.sleep(100);
        ...
        for (int i = 0; i < count; i++) {
            ...
            bmsg.setJMSReplyTo(tempResponseQ);
            prod.send(bmsg);
            if (++commitcount == sendcommit) {
                sess.commit();
                commitcount = 0;
            }
            synchronized (TemporaryQueueTest.class) {
                sendcount++;
                if (sendcount % 500 == 0)
                    try {
                        TemporaryQueueTest.class.wait(3);
                    } catch (Exception e) {
                    }
            }
        }
        ...
        synchronized (TemporaryQueueTest.class) {
            ...
            while ((wait-- > 0) && (!recvdone || !echodone)) {
                try {
                    TemporaryQueueTest.class.wait(50);
                } catch (Exception e) {
                }
            }
    ...
    public void doReceive() throws JMSException {
        ...
        MessageConsumer cons = sess.createConsumer(tempResponseQ);
        byte[] b = new byte[1];
        int commitcount = 0;
        int i = 0;
    ...
    public void doEcho() throws JMSException {
        ...
        do {
            ...
            MessageProducer prod = getReplyToProd(sess, responseQ);
            echocount++;
            prod.send(msg);
            if (++commitcount == echocommit) {
                sess.commit();
     ...
     */
    MessageProducer getReplyToProd(Session sess, Queue queue) throws JMSException {
        if (replytoProds.containsKey(queue))
            return (replytoProds.get(queue));
        MessageProducer prod = sess.createProducer(queue);
        prod.setDeliveryMode(DeliveryMode.PERSISTENT);
        replytoProds.put(queue, prod);
        return prod;
    }
    ...
    public static void main(String[] args) {
         ...
         */
        new Thread(new TemporaryQueueTest(ECHO)).start();
