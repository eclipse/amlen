 ...
 */
public class AsyncTimerTest implements Runnable {
	int  action;
	 ...
	 */
    public AsyncTimerTest(int action) {
    	this.action = action;
    }
    ...
    public static void main(String [] args) {
    	...
    	for (i=0; i<thread_count; i++) {
    		new Thread(new AsyncTimerTest(i)).start();
    	}
		...
		public void onMessage(Message msg) {
			...
			try {
	    		...
	    		if (bmsg.getBodyLength() != msglen || b[0] != (byte)recvcount) {
	    			throw new JMSException("Message received is not the correct message: "+recvcount+" message length "+bmsg.getBodyLength());
	    		}
	    		recvcount++;
	    		if (recvcount == count) {
					recvdone = true;
