...
public class TopicTest implements Runnable{
    ...
    public static long doSend() throws JMSException {
        ...
        try {
            for (i=0; i<6; i++) {
                sess.createConsumer(sess.createTopic("Junk" + i));
            }
        } catch (Exception e) {
    ...
    public void doReceive() throws JMSException {
        ...
        Topic responseT = sess.createTopic("ResponseT");
        ((ImaProperties)responseT).put("DisableACK", true);
        MessageConsumer cons = sess.createConsumer(responseT);
    ...
    public void doEcho() throws JMSException {
        ...
        MessageProducer prod = sess.createProducer(responseT);
        prod.setDeliveryMode(DeliveryMode.NON_PERSISTENT);
        byte [] b = new byte [1];
        int i = 0;
    ...
    public static void main(String [] args) {
        ...
        try {
            ...
            String suites = System.getenv("IMACipherSuites");
            String protocol = System.getenv("IMASecurityProtocols");
            if (server == null)
                server = "127.0.0.1";
            if (port == null)
                port = "16102";
            ...
            if (suites != null)
                ((ImaProperties)fact).put("CipherSuites", suites);
            if (protocol != null)
                ((ImaProperties)fact).put("SecurityProtocols", protocol);
            conn = fact.createConnection();
            //conn = fact.createConnection();
            conn.setClientID("Thisisatestthisisonlyatestbuthestringisquitelong");
