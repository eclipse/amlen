...
#
# Description:
#   This script executes ISM test bucket for:  [Setting up the Policy and ISM Admin Objects for Automation].
#
# Input Parameters:
...
# copied from FVT scripts... make sure containers are running.
appliance=1
while [ ${appliance} -le ${A_COUNT} ]
do
    ../scripts/serverControl.sh -a startServer -i ${appliance}
    ((appliance+=1))
done
appliance=1
while [ ${appliance} -le ${A_COUNT} ]
do
    ../scripts/serverControl.sh -a checkStatus -i ${appliance} -t 15 -s production
    ((appliance+=1))
done
...
# Test Bucket Scenarios (group 1) TO ISM as KVM
RUNCMD="../scripts/run-scenarios.sh  ism-SVT_ISM_ENV-runScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD} with environment variable AUTOMATION_FRAMEWORK_RESTART_SERVERS=0
export AUTOMATION_FRAMEWORK_RESTART_SERVERS=0 # RTC 70108 
$RUNCMD
run01Result=$?

# Test Bucket Scenarios (group 2)  TO ISM as application
###RUNCMD="../scripts/run-scenarios.sh  ../common/ism-websockets-runScenarios02.sh ${logfile}"
...
###exit $( expr ${run01Result} + ${run02Result} )
exit $( expr ${run01Result} )
