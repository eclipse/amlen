...
def installRarSingle(node):
    ...
    if rId == '':
        AdminTask.installResourceAdapter('[-nodeName '+node+' -rarPath '+rarFile+' -rar.name "'+raname+'" -rar.archivePath ${CONNECTOR_INSTALL_ROOT} -rar.classpath /WAS/JMSTestDriver.jar -rar.DeleteSourceRar false]')
