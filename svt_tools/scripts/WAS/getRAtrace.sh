...
if [[ "$1" == "all" ]] ; then
    if [[ "${srvdef}" =~ "SINGLE" ]] ; then
        ...
        for node in ${array} ; do
            ...
            echo "bzip2 -k -9 ${WASPath}/profiles/AppSrv01/logs/server3/SystemOut.log" | ssh ${WASAddress}
            scp ${node}:${WASPath}/profiles/AppSrv01/logs/server3/SystemOut.log.bz2 ./SystemOut.server3.log.bz2.log
            
            echo "bzip2 -k -9 ${WASPath}/profiles/AppSrv02/logs/server3/SystemOut.log" | ssh ${WASAddress}
