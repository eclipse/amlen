...
CMD='ssh admin@'${SERVER}' "imaserver stat Subscription StatType=PublishedMsgsHighest ResultCount=100" | tail -n +2'
...
while [ ${#OUT[@]} -gt 0 ]; do
  for LINE in ${OUT[@]}; do
    if [ ${#LINE} -gt 3 ]; then
      ...
      CMD='ssh admin@'${SERVER}' "imaserver delete Subscription \"SubscriptionName='${SN}'\" \"ClientID='${CN}'\""'
      display "${CMD}" ${SCREENOUT_FILE}
      if [ ! -z ${SCREENOUT_FILE} ]; then
