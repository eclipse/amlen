...
### Changes to suppport adding one hundred members at a time 
###  PLEASE BESURE YOU GIVE Full UID number without leading zero and realize 00-99 will replace the last two digits of that RANGE
###
UIDPREFIX="u"
FIRST_UID=100
LAST_UID=999
...
#set -x
GROUPDN="dn: cn=svtUsersInternet,ou=groups,${BASE_DN}"
firstloop=0

# Create the leading FIVE digits of a FULL USERNAME's UID
set -x
   FIRST_UID_UNTRIMMED=`printf "%07d" ${FIRST_UID}`
   FIRST_UID_ROOT=${FIRST_UID_UNTRIMMED:0:5}
   LAST_UID_UNTRIMMED=`printf "%07d" ${LAST_UID}`
   LAST_UID_ROOT=${LAST_UID_UNTRIMMED:0:5}
set +x
...
do
   ...
   # Create the trailing TWO digits of a FULL USERNAME's UID
   for userSuffix in {0..99}
   do
      UID_SUFFIX=`printf "%02d" $userSuffix`
