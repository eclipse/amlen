...
if [ "$#" -ne "6" ]; then
    ...
    echo "For examples,"
    echo "    To create an LDIF file to remove DN uid=u0000001,ou=users,ou=SVT,o=IBM,c=US from 1000 groups(G0000001 .. G0001000):"
...
LDIF_FILE="removeMembership.${USER_NAME}.${GROUP_PREFIX}.${FIRST_GID}.${LAST_GID}.ldif"
...
if [ "${entries}" -gt 9999 ]; then
    ...
    do
        ...
        filename="remove.${GROUP_PREFIX}.${first}.${last}.tmp"
        rm -f ${filename}
