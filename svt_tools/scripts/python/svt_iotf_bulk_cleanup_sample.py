...
#    print(str(item))
    i = 0
    try: 
        logger.debug("len of item is {0}".format(len(item)))
        if len(item) > 1:
            logger.info("len of payload is {0}".format(len(payload)))
            if len(payload) > 1:
                logger.info("clearing payload")
                payload.clear()
                for thisDevice in item:
                    payload.append({"id": thisDevice.deviceId,"type":thisDevice.deviceType,"password":thisDevice.devicePw})
            else:
    ...
    url = "https://"+ orgId + "."+apiUrl +  "/bulk/devices/delete"
    logger.info(url)
...
#        return
        ...
        if r.status_code == 202 or r.status_code == 201:
            ...
            logger.critical("Response content " + r.text+str(r.headers))
...
def worker():
    ...
    while not workQ.empty():
        ...
        for deviceCount in range(batchSize) : 
            device = workQ.get()
