	...
	public boolean runTest(ImsLDAPTestData testData) {
		...
		if (testData.isCLICreateLDAP()) {
			try {
				...
				CLICommand cmd = imaConfig.createLDAP(testData.getLDAPCommandOptions());
				if (testData.isDebug()) {
		...
		try {
			if (testData.isCLICreateLDAP()) {
				//TODO: Login the Web UI to verify that LDAP was created
				...
				WebBrowser.maximize();
				LDAPTasks.loadURL(testData);
				LDAPTasks.addLDAP(testData);
				LoginTasks.logout();
			}
			result = true;
		} catch (Exception e){
