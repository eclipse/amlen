...
import com.ibm.ism.test.tasks.ManipulateAddUsersTasks;
import com.ibm.ism.test.tasks.ManipulateDeleteUsersTasks;
import com.ibm.ism.test.tasks.license.LicenseAgreementTasks;

/**
 * This test validates the functions of the Users.
 */
public class Appliance_Users_Test1 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//Log in as Appliance Admin to create new user
			...
			System.out.println("Appliance Users Test1");
			VisualReporter.logInfo("Appliance Users Test1");
			
			ManipulateAddUsersTasks.navigateAppliance(null);
			IsmTest.delay ();
			...
			//Log in as sheila
			IsmTest.gsUserId = "sheila";
			IsmTest.gsPassword = "nyc4meto";
			IsmTest.gsUserRole = IsmTest.APPLIANCE_ADMIN_ROLE;
			LoginTasks.login(IsmTest.gsUserId, IsmTest.gsPassword);
			...
			VisualReporter.logInfo("Second Login");
			...
			//Login as Admin to delete New User
			WebBrowser.start(browser);
			WebBrowser.loadUrl(url);
			IsmTest.waitForBrowserReady();
			...
			System.out.println("Third Login");
			VisualReporter.logInfo("Third Login");
			

			ManipulateDeleteUsersTasks.navigateAppliance(null);
			IsmTest.delay ();
			

			LoginTasks.logout();

			//Attempt to Login as New User who was previously deleted
			WebBrowser.start(browser);
			WebBrowser.loadUrl(url);
			IsmTest.waitForBrowserReady();
			...
			System.out.println("Fourth Login");
			VisualReporter.logInfo("Fourth Login");
