...
protected:
     ...
     * @param readOnly
     * @param wrap if false reallocates and copies the source buffer, else wraps it.
     *
     * @throw OutOfMemoryException if not enough memory.
     */
    ByteBuffer(const char* buffer, size_t length, bool readOnly, bool wrap = false);
...
//    void writeStringSet2Pos(size_t pos, const StringSet &strSet)   {writeStringSet(pos, strSet);}
//    void writeByteArray2Pos(size_t pos, const char* ba, size_t length)    {writeByteArray(pos, ba, length);}
//    void writeByteBuffer2Pos(size_t pos, const ByteBuffer &bb, size_t length)    {writeByteBuffer(pos, bb, length);}
...
public:
    ...
    /**
     * Create a read-only buffer from a source buffer (copy or wrap).
     *
     * @param buffer
     * 		source buffer
     * @param
    ...
    static boost::shared_ptr<ByteBuffer> createReadOnlyByteBuffer(
        		const char* buffer, size_type length, bool wrap);
...
//	}
	...
	void writeNodeVersion(const NodeVersion& version);

	void writeNodeID(const NodeIDImpl_SPtr& nodeID);

	void writeVirtualID(const util::VirtualID& vid);

	void writeStreamID(const messaging::StreamIDImpl& streamID);
...
//    double          readDouble(void)                    {return readDouble(_position);}
    ...
    util::VirtualID readVirtualID();

    util::VirtualID_SPtr readVirtualID_SPtr();

    messaging::StreamIDImpl readStreamID();

    messaging::StreamID_SPtr readStreamID_SPtr();
...
//	 * Slice the buffer.
//	 *
//	 * Set a virtual slice of this buffer.
//	 * The slice starts at 'start' (inc.) and has 'length' bytes.
//	 * Sets the virtual base to start and capacity to 'length'.
//	 *
//	 * @param start
//	 * @param length
//	 *
...
//	 */
//	void slice(size_t start, size_t length);
//
//	/**
//	 * Detach storage from the buffer.
//	 *
//	 * Returns a pointer to the starting point of the virtual buffer slice,
//	 * reallocates storage if necessary. Note that the returned buffer may
//	 * be bigger than the slice.
//	 *
//	 *	The ByteBuffer remains with a NULL buffer.
//	 *
//	 * @return
//	 */
//	char* extractBufferSlice();
...
//	 * @return
//	 */
//boost::tuples::tuple<char*,size_t,size_t,size_t> extractBuffer();
