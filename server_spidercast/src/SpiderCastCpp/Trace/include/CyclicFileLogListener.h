 ...
 *
 */
class CyclicFileLogListener : public log::LogListener
{
public:
	/**
	 * Log listener that writes to a file, with support for cyclic log files of limited size.
	 ...
	 * @return
	 */
	CyclicFileLogListener(const char* logFileNameBase, const char* logFileNameSuffix,
			int numFiles, int maxFileSizeKB = 10000);

	virtual ~CyclicFileLogListener();
	...
	static void logListener(void *user, int log_level, const char *id,
			const char *message)
	{
		CyclicFileLogListener* listener = (CyclicFileLogListener*) user;
		listener->print(static_cast<log::Level>(log_level), id, message);
	}
...
private:
	...
	void print(log::Level log_level, const char *id, const char *message);
