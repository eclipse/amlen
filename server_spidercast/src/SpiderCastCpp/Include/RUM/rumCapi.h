...
#define   rumConnectionID_t  rum_uint64
 ...
 /** \brief define a type for TurboFlow label         */
#define   rumtfLabel_t        unsigned int
...
typedef enum
{
   ...
   RUM_RELIABLE_FAILOVER_PRIM     = 5,             /**< failover acting as primary                        */
   RUM_RELIABLE_FAILOVER_BU       = 6,             /**< failover acting as backup reconnect mode          */
   RUM_RELIABLE_FAILOVER_BU_RPLY  = 7              /**< failover acting as backup replay mode             */
...
typedef enum
{
   ...
   RUM_RELIABLE_FAILOVER          = 4              /**< receive failover stream (reliable and ordered)    */
}RUM_Rx_RELIABILITY_t ;

/** \brief Constants used to define the way LLM use the message filtering definitions (TurboFlow mode and message Properties)
     of an incoming stream that is accepted on a Queue receiver.
 */
typedef enum
{
   RUM_FILTER_BY_STREAM_DISABLED  = 0,             /**< Message filtering on a stream is performed
                                                    *   according to the definitions of the Queue.
                                                    *   The TurboFlow mode and Properties usage of an
                                                    *   incoming stream must match those of the Queue
                                                    *   receiver accepting the stream                     */
   RUM_FILTER_BY_STREAM_TF        = 1,             /**< Message filtering on a stream is performed
                                                    *   according to the TurboFlow mode of the stream
                                                    *   and the Properties usage definition of the Queue.
                                                    *   The TurboFlow mode of an incoming stream can be
                                                    *   different from that of the Queue accepting it     */
   RUM_FILTER_BY_STREAM_PROPS     = 2,             /**< Message filtering on a stream is performed
                                                    *   according to the Properties usage of the stream
                                                    *   and the TurboFlow mode of the Queue.
                                                    *   The Properties usage of an incoming stream can be
                                                    *   different from that of the Queue accepting it     */
   RUM_FILTER_BY_STREAM_ALL       = 9              /**< Message filtering on a stream is performed
                                                    *   according to the definitions of the stream.
                                                    *   The TurboFlow mode and Properties usage of an
                                                    *   incoming stream can be different from those of
                                                    *   the Queue receiver accepting the stream           */
}RUM_FILTER_BY_STREAM_MODE_t ;

/** \brief TurboFlow mode */
typedef enum
{
   RUM_TF_MODE_DISABLED           = 0,             /**< Turbo flow disabled                               */
   RUM_TF_MODE_LABEL              = 1,             /**< Turbo flow enabled using integer labels           */
   RUM_TF_MODE_BITMAP             = 2              /**< Turbo flow enabled using bitmap                   */
}RUM_TF_MODE_t ;
...
typedef struct
{
  rumStreamID_t             stream_id ;                       /**< Stream unique ID                           */
        ...
        char                pad[sizeof(void *)] ;             /**< Reserved - Alignment pad                   */
        RUM_TF_MODE_t       turbo_flow_mode ;                 /**< Turbo flow mode of stream                  */
...
typedef struct
{
   rumMessageID_t           msg_sqn ;                         /**< Message sequence number                    */
    ...
    rumtfLabel_t            msg_label ;                       /**< Message TurboFlow label                    */
...
typedef struct
{
   ...
   rumMessageID_t          *msg_sqn ;                         /**< Output parameter - if not NULL RUM returns the sequence number of the submitted message.
                                                                   Note that message sequence numbers are valid only on Failover queues                    */
   rumtfLabel_t            *msg_label ;                       /**< TurboFlow label (ignored if turbo_flow_mode is not in label mode)                       */
        char               *msg_buf ;                         /**< Message data                                                                            */
...
typedef struct
{
        ...
        RUM_TF_MODE_t          turbo_flow_mode ;                        /**< is the stream a TurboFlow stream                           */
...
typedef struct
{
        ...
        RUM_TF_MODE_t          turbo_flow_mode ;                     /**< TurboFlow mode for the queue. Default value RUM_TF_MODE_DISABLED  */
        ...
        int                    app_controlled_batch ;                /**< If set, RUM does not attempt to send partial packets.
                                                                      ...
                                                                      *   rumTxMessage or the rumTFlushMessageBuffer API; if the
...
typedef struct
{
        ...
        int                    stream_join_backtrack ;               /**< Number of packets that the receiver will attempt to request
                                                                      ...
                                                                      *   request any packets                                               */
  RUM_FILTER_BY_STREAM_MODE_t  filter_by_stream_mode ;               /**< A Queue receiver can be configured to filter messages based on
                                                                      *   TurboFlow (turbo_flow_mode) and/or message Properties
                                                                      *   (enable_msg_properties). By default a Queue receiver will only
                                                                      *   accept incoming streams that exactly match its message filtering
                                                                      *   definition, e.g., if the Queue was configured with TurboFlow label
                                                                      *   and no properties the incoming stream must be configured in
                                                                      *   exactly the same. The filter_by_stream_mode field allows the user
                                                                      *   to instruct LLM to use the message filtering definition of the
                                                                      *   stream rather than those of the Queue receiver.
                                                                      *   Default value RUM_FILTER_BY_STREAM_DISABLED                       */
...
typedef struct
{
        ...
        int               turbo_flow_mode ;                 /**< TurboFlow mode of the stream                           */
...
typedef struct
{
        ...
        unsigned int      packets_duplicate ;               /**< Number of duplicate packets from this stream since the 
                                                             *   stream was accepted or the stream statistics were 
                                                             *   reset                                                  */
        int               ha_failovers_recorded ;           /**< Number of failovers recorded for this stream since the 
                                                             *   stream was accepted or the stream statistics were 
                                                             *   reset                                                  */
        unsigned int      ha_repair_requested ;             /**< Repair packets requested during failover for this 
                                                             *   stream since the stream was accepted or the stream 
                                                             *   statistics were reset                                  */
        char              stream_id_str[24] ;               /**< String representation of stream ID                     */
        char              queue_name[RUM_MAX_QUEUE_NAME] ;  /**< The queue Name of the stream                           */
...
typedef struct
{
        ...
        int              reliability ;                     /**< Reliability level                                      */
        int              turbo_flow_mode ;                 /**< The TurboFlow mode of the Queue receiver               */
        int              tf_labels_accepted ;              /**< Number of TurboFlow labels accepted                    */
        unsigned int     ha_messages_lost ;                /**< Number of messages lost on Failover (HA) streams 
                                                            *   accepted by this queue since the queue was created or  
...
typedef struct
{
   ...
   const char *instanceName ;                              /**< Name of the rum instance                               */
        int    CCTLevel;                                   /**< Coordinated Cluster Time Level. 0 for disable.     
                                                            *   1 for enable for Latency Monitoring. 2 for enable    
                                                            *   for Latency Monitoring and Logging. =                  */
  const char  *CCTCluster;                                 /**< The name of the Coordinated Cluster Time cluster       */ 
        char   pad[sizeof(void *)] ;                       /**< Reserved - Alignment pad                               */
...
/**
 * \brief Transmitter Latency Monitoring information. Used in rumTxGetLatencyInfo.
 *  Includes a list of several histogram. If the application is not interested in a particular histograms it can set the pointer
 *  for that histogram to NULL. Note that depending on the monitoring level some histograms may not be available
 */
typedef struct
{
  rumHistogram               *hist_msgQ ;                 /**< Histogram recording the number of messages in a packet (when the
                                                           *   packet is sent).
                                                           *   This histogram is available at monitor level LOW or higher             */
  rumHistogram               *hist_packetQ ;              /**< Histogram recording the number of packets that are found in the queue
                                                           *   of pending packets waiting to be sent.
                                                           *   This histogram is available at monitor level LOW or higher             */
  rumHistogram               *hist_submit2sent ;          /**< Histogram recording the latency from the time the first message is
                                                           *   placed in a packet until the packet is sent (and stored in history).
                                                           *   This histogram is available at monitor level MEDIUM or higher          */
  rumHistogram               *hist_submit2pack ;          /**< Histogram recording the latency from the time the first message is
                                                           *   placed in a packet until the packet is closed and placed in the
                                                           *   pending queue.
                                                           *   This histogram is available only at monitor level HIGH                 */
  char                        rsrv[64] ;                  /**< Reserved                                                               */
}rumTxLatencyInfo ;

/** \brief Receiver Latency Monitoring information. Used in rumRxGetLatencyInfo.
 *  Includes a list of several histogram. If the application is not interested in a particular histograms it can set the pointer
 *  for that histogram to NULL. Note that depending on the monitoring level some histograms may not be available
 */
typedef struct
{
  rumHistogram               *hist_packetQ ;              /**< Histogram recording the number of packets in the queue used to store
                                                           *   packets that have been received and are ready to be processed by the
                                                           *   application.
                                                           *   This histogram is available at monitor level LOW or higher             */
  rumHistogram               *hist_receive2process   ;    /**< Histogram recording the latency from the time a packet is received
                                                           *   until that packet is first processed by the application.
                                                           *   This histogram is available at monitor level MEDIUM or higher          */
  rumHistogram               *hist_appQ2process ;         /**< Histogram recording the latency from the time a packet is placed in
                                                           *   the queue of packets ready to be processed by the application until
                                                           *   the packet is first processed by the application.
                                                           *   This histogram is available only at monitor level HIGH                 */
  rumHistogram               *hist_sent2receive ;         /**< Histogram recording the latency from the time a packet has been sent
                                                           *   until the packet is received. This histogram represent latency of the
                                                           *   network (including the OS networking stack). It is important to note
                                                           *   that the accuracy of this histogram depends on the synchronization of
                                                           *   the clocks used at the sending and receiving hosts.
                                                           *   This histogram is available only at monitor level HIGH and only if
                                                           *   the flag deliver_timestamp was set at the transmitter                  */
  char                        rsrv[64] ;                  /**< Reserved                                                               */
}rumRxLatencyInfo ;
...
/**
 * Same as rumInit() but uses default configuration and log function.
 * A default RUM implementation of on_log_event() will be used.
 * If a configuration file named rumBasic.cfg exists in the local directory,
 * configuration parameters will be read from that file, otherwise default configuration
 * parameters will be used.
 * At most one default RUM instance can be created.
 * @param rum_instance Pointer to structure where instance handle will be created (output)
 * @param rc  Pointer to integer where failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumInitDefaultConfig(rumInstance *rum_instance, int *rc);
...
/**
 * Provides the current time according to the latency monitoring clock used by the RUM receiver instance.
 * The latency monitoring clock can be the RUM internal clock, the Coordinated Cluster Time (CCT), or the external clock the
 * application provided as the get_time callback in rumLatencyMonParams.
 * The time is provided as a double representing a value in seconds.
 * If the application provided an external clock for latency monitoring it is more efficient for the application to call the
 * external clock directly rather than using this API.
 * This API can be used to obtain the elapsed time from an RUM time stamp that was generated by the latency monitoring clock.
 * In the case of the receive_time_stamp in the rumPacket structure the time returned by this API can be used to calculate the
 * elapsed time since both the receive_time_stamp and the call to this API are using the same clock.
 * @param rum_instance An initialized Receiver instance
 * @param time - A pointer to a double where the current time will be written. The time is provided by the RUM internal clock as a double
 *               representing a value in seconds (output)
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumGetTime(const rumInstance *rum_instance, double *time, int *rc);
...
/**
 * Change run-time modifiable configuration parameters of the RUM instance.
 * Not all configuration parameters can be modified at run time.
 * Currently the list of parameters that can be changed using this function
 * include: LogLevel (for advance logging), SnapshotCycleMilli_tx, SnapshotCycleMilli_rx, PrintStreamInfo_tx, PrintStreamInfo_rx.
 * @param rum_instance  An initialized RUM instance to modify (input).
 * @param n_params  Number of parameters in the config_params list.
 * @param config_params  List of configuration parameters, each in the form of "config_param=config_value".
 * @param rc  Pointer to integer where failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int rumChangeConfigurationParameters(const rumInstance *rum_instance, int n_params, char **config_params, int *rc);
...
/**
 * Deprecated - use rumInitStructureParameters() instead.<br>
 * Initialize a structure that is passed by the application to RUM in one of the APIs.
 * Will set appropriate initial values in the structure. The application then only needs to change the fields it is interested in.
 * If an rum_instance is provided the initial values will be set according to the instance configuration.
 * @param rum_instance  A pointer to an initialized RUM instance or NULL if instance is not relevant (input)
 * @param structure_id  Identifier of the structure (input)
 * @param structure  Structure to be initialized (output)
 * @param rc  Pointer to integer where failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int rumSetStructureParameters(const rumInstance *rum_instance, RUM_STRUCTURE_ID_t structure_id , void *structure, int *rc);


/* Statistics related Functions */

/**
 * Get the statistics for an RUM instance.
 * <br> See rumStats <br>
 * @param rum_instance An initialized RUM instance (input)
 * @param report Pointer to rumStats structure that will be populated (output)
 * @param record_stream_info Indicates whether individual stream statistics should be recorded in addition to the general RUM statistics
 * @param record_conn_info Indicates whether individual connection statistics should be recorded in addition to the general RUM statistics
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumGetStatistics(const rumInstance *rum_instance, rumStats *report, int record_stream_info, int record_conn_info, int *rc);

/**
 * Reset the running counters in the statistics of the RUM instance and the statistics of all active queues of this instance.
 * The fields in rumStats which are reset by this API are documented as maintaining the counter  
 * "since the RUM instance was created or the instance statistics were reset", e.g., messages_sent and bytes_sent.
 * @param rum_instance An initialized RUM instance (input)
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumResetStatistics(const rumInstance *rum_instance, int *rc);

/**
 * Get configuration statistics for an RUM instance
 * <br> see rumRConfigStats <br>
 * @param rum_instance An initialized RUM instance (input)
 * @param config Pointer to rumConfigStats structure that will be populated (output)
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumGetConfigurationStatistics(const rumInstance *rum_instance, rumConfigStats *config, int *rc);
 ...
 * @param rum_instance  An initialized RUM instance structure from rumInit() or rumInitDefaultConfig() (input)
...
/**
 * Place any pending messages in the Queue's message buffer in a packet and queue the packet for transmission.
 * @param queue_t An initialized Queue transmitter structure (input)
 * @param rc  Pointer to integer where failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumTFlushMessageBuffer(const rumQueueT *queue_t, int *rc) ;

/* HA Tx Functions */
/**
 * Changes the reliability of the queue_t to the new reliability defined by the reliability parameter.
 * This function can only be applied to a queue_t with failover reliability.
 * @param queue_t  An initialized Queue transmitter structure with failover reliability.
 * @param reliability Queue reliability level
 * @param rc  Pointer to integer where failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int rumTChangeReliability(const rumQueueT *queue_t, RUM_Tx_RELIABILITY_t reliability, int *rc);

/**
 * Sets the initial sequence number from which to start numbering messages.
 * By default the first message is given sequence number 1.
 * It is strongly advised that the application use this API to set the message sequence number on the queue_t only BEFORE
 * the first message has been submitted on the queue_t. If the application sets the message number after messages have
 * been submitted, a gap in the sequence numbers of the message stream will be introduced which may violate the reliability
 * of the message delivery at the receivers of the stream.
 * This function can only be applied to a queue_t with failover reliability.
 * @param queue_t  An initialized Queue transmitter structure with failover reliability (input)
 * @param msg_id  New message number (input)
 * @param rc  Pointer to integer where failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int rumTSetMessageNumber(const rumQueueT *queue_t, rumMessageID_t msg_id, int *rc) ;

/**
 * Updates the late join mark of the queue_t to the value given in lj_mark.
 * This function can be called only on a late join queue_t.
 * The late join mark defines the earliest packet sequence number that a late join receiver
 * (i.e., a receiver that accepts this queue after data had been sent on it) can request when accepting this queue.
 * When a late join queue is created the late join mark is set to 1.
 * If the late join mark is smaller than the queue's transmission window trail then the trail
 * defines the actual late join mark that new receivers will use. 
 * The late join mark has no effect on history cleaning.
 * @param queue_t  An initialized Queue transmitter structure with late join enabled (input).
 * @param lj_mark  New late join mark (input).
 * @param rc  Pointer to integer where failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumTSetLateJoinMark(const rumQueueT *queue_t, unsigned int lj_mark, int *rc);

/**
 * This API can be used to replace some of the callbacks (and their associated user) that were set for an existing queue_t.
 * Only callbacks and users that were set when the queue_t was created (using rumTCreateQueue()) can be replaced!
 * The callbacks and associated user that can be changed are for the on_event callback.
 * Hence, only the following fields in the rumTxQueueParameters structure are inspected (all other fields are ignored)
 * 1. on_event and event_user
 * Only non NULL pointers are replaced, the caller can use this API to replace either the callback or the user or both.
 * No synchronization is done with the thread that might currently be using the previous pointers,
 * so no guaranty is given to when the calling thread is actually switched to the new pointers.
 * @param queue_t  An initialized transmitter queue structure.
 * @param params - Initialized rumTxQueueParameters structure with the requested callbacks and users set.
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumTReplaceCallbacks(const rumQueueT *queue_t, const rumTxQueueParameters *params, int *rc);
...
/**
 * Get statistics of a transmitter queue.
 * <br> See rumTStreamStats <br>
 * @param queue_t  An initialized Queue transmitter structure (input)
 * @param report  Pointer to rumTStreamStats that will be populated (output)
 * @param rc  Pointer to integer where failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
...
/**
 * Reset running counters in the statistics of an RUM transmitter queue.
 * The fields in rumTStreamStats which are reset by this API are documented as maintaining the counter  
 * "since the stream was created or the stream statistics were reset", e.g., messages_sent and bytes_sent.
 * The queue must be active, i.e., not closed.
 * @param queue_t  An initialized Queue transmitter structure (input)
 * @param rc  Pointer to integer where failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
...
/**
 * Receive a packet from the Queue queue_r in a blocking or non-blocking fashion.
 * A packet may include multiple messages (the number of messages is indicated by the num_msgs field
 * in the packet structure). The messages are referenced using the rumRxMessage elements stored in
 * the msgs_info array of the packet.
 * If packet points to NULL it means that there are no packets to receive on queue_r (note that this
 * is not considered an error).
 * It is the application's responsibility to return the packet using the rumRReturnPacket() API; failure
 * to do so will cause a memory leak and may cause the RUM to freeze.
 * The packet structure should be considered as read-only; the application must not change any of its fields.
 * This API may be called only on a Queue on which an on_data callback has been registered.
 * If the block parameter is set to zero RUM will not block trying to return a packet (if the queue is
 * empty packet will points to NULL); any other value would indicate RUM to block until a packet is
 * available (or the queue has been closed).
 * @param queue_r  An initialized Receiver queue structure.
 * @param packet  Pointer to address of packet location.
 * @param block  Indicates whether to block while waiting for a packet to arrive.
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRReceivePacket(const rumQueueR *queue_r, rumPacket **packet, int block, int *rc) ;

/**
 * Returns an RUM packet that has been received using rumRReceivePacket() API.
 * The application must return every packet that has been received. Packets contain
 * memory allocated from pools; it is therefore recommended that the application
 * should not maintain a large number of packets as this may deplete resources.
 * The application may not hold any packets when the RUM is stopped; all packets
 * should be freed before calling rumStop().
 * @param packet  Pointer to packet returned from rumRReceivePacket() operation.
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRReturnPacket(rumPacket *packet, int *rc) ;


/**
 * This API can be used to replace some of the callbacks (and their associated user) that were set for an existing queue_r.
 * Only callbacks and users that were set when the queue_r was created (using rumRCreateQueue) can be replaced!
 * The callbacks and associated user that can be changed are for the accept_stream, on_event and on_message callbacks.
 * Hence, only the following fields in the rumRxQueueParameters structure are inspected (all other fields are ignored)
 * 1. accept_stream and accept_user
 * 2. on_event and event_user
 * 3. on_message and user
 * Only non NULL pointers are replaced, the caller can use this API to replace either the callback or the user or both.
 * No synchronization is done with the thread that might currently be using the previous pointers,
 * so no guaranty is given to when the calling thread is actually switched to the new pointers.
 * @param queue_r  An initialized receiver queue structure.
 * @param params - Initialized rumRxQueueParameters structure with the requested callbacks and users set.
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRReplaceCallbacks(const rumQueueR *queue_r, const rumRxQueueParameters *params, int *rc);
...
/**
 * Change the reliability of a receiver queue to go-back replay mode.
 * This mode affects the way RUM acts when a replay backup stream (received by this queue_r)
 * changes to a primary stream. In go-back replay mode RUM will start delivering messages from this stream
 * starting from the message sequence number that was received (approximately)
 * goback_time_milli milliseconds before the failure occurred, and will not filter
 * duplicate messages. This is in contrast to conventional replay mode where RUM delivers
 * messages starting from the last sequence number that has been received and filters all duplicates.
 * It is legal to call this function more than once on the same queue in order to change the go-back
 * time. Note, however, that RUM requires a certain time period (roughly three times goback_time_milli)
 * to be ready to process a failover according to the new time.
 * This function can only be applied to a queue_r with failover reliability.
 * The value of goback_time_milli must be greater than zero.
 * @param queue_r An initialized receiver queue structure with failover reliability (i.e. RUM_RELIABLE_FAILOVER)
 * @param goback_time_milli Milliseconds worth of data to retrieve from new primary stream after failover
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int rumRSetGobackReplay(const rumQueueR *queue_r, int goback_time_milli, int *rc);

/* Turbo Flow Rx Functions */

/**
 * Set the bitmap filter that is used to filter messages when the queue is configured to work in TurboFlow bitmap mode (RUM_TF_MODE_BITMAP).
 * This call can be invoked more than once. Every call replaces the currently set bitmap filter.
 * Before the first call RUM uses a filter that does not pass any messages (equivalent to all zeros bitmap).
 * RUM will filter messages by performing a bitwise AND between the bitmap provided in the message and the bitmap filter the application provided;
 * only messages for which the AND operation yields a result different than zero will be delivered to the application.
 * If the length of the bitmap in the message and the length of the filter bitmap the receiver has are different,
 * the first bytes of the shorter bitmap will be padded with zeros to match the length.
 * If 'bitmap_length >= RUM_MAX_BITMAP_LENGTH' and the first RUM_MAX_BITMAP_LENGTH bytes of 'bitmap' are all 0xff then the queue is set to 'accept all' mode and
 * accepts all messages without checking the bitmap in the incoming message, i.e., even if the incoming bitmap is all zeros.
 * If 'bitmap_length <= 0' then the queue is set to 'reject all' mode and rejects all messages (equivalent to the initial state or an all zeros bitmap).
 * If the application is busy processing messages of the queue_r the function may return RUM_BUSY
 * indicating that operation cannot be completed. In this case the application should try to call this
 * function again.
 * @param queue_r An initialized Receiver queue or streamset set to work in TurboFlow Bitmap mode (RUM_TF_MODE_BITMAP).
 * @param bitmap_length The length of the bitmap filter (input)
 * @param bitmap The new bitmap filter to use (input)
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success (filter set), RUM_BUSY if the operation cannot be completed due to messages
 * (of queue_r) being processed by the application, or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRSetBitmapFilter(const rumQueueR *queue_r, int bitmap_length, rumtfBitMap_t bitmap, int *rc);

/**
 * Get the turboflow bitmap filter for a message.
 *
 * This call AND the use of the returned values must be done during a call to on_message(), on_packet(),
 * or after a packet has been received using rumRRecievePacket() and before it is returned using rumRReturnPacket().
 * <p>
 * If the hdr_len field in the rumRxMessage structure is zero, or if the turboflow type of the
 * stream is not equal to RUM_TF_MODE_BITMAP, there will be no bitmap and the length will be returned
 * as zero.
 * @param msg    A receive message structure
 * @param bitmap_length The length of the bitmap filter (output)
 * @param bitmap The bitmap filter of the message (output)
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRGetMsgBitmap(rumRxMessage * msg, int * bitmap_length, rumtfBitMap_t * bitmap, int * rc);

/**
 * Indicates that messages with the label given in the label parameter should be
 * accepted, i.e., delivered to the application. This function can be applied
 * only to TurboFlow queues or streamsets.
 * If the application is busy processing messages of the queue_r the function may return RUM_BUSY
 * indicating that operation cannot be completed.In this case the application should try to call this
 * function again.
 * @param queue_r An initialized receiver queue or streamset set to work in TurboFlow Label mode (RUM_TF_MODE_LABEL).
 * @param label Label value to accept on this queue (input)
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success (label added), RUM_BUSY if the operation cannot be completed due to messages
 * (of queue_r) being processed by the application, or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRAddAcceptedFlow(const rumQueueR *queue_r, rumtfLabel_t label, int *rc);

/**
 * Indicates that messages with the label given in the label parameter should be
 * filtered, i.e., dropped by RUM and not delivered to the application.
 * This function can be applied only to TurboFlow queues or streamsets.
 * If the application is busy processing messages of the queue_r the function may return RUM_BUSY
 * indicating that operation cannot be completed.In this case the application should try to call this
 * function again.
 * @param queue_r An initialized receiver queue or streamset set to work in TurboFlow Label mode (RUM_TF_MODE_LABEL).
 * @param label Label value to filter on this queue
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success (label removed), RUM_BUSY if the operation cannot be completed due to messages
 * (of queue_r) being processed by the application, or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRRemoveAcceptedFlow(const rumQueueR *queue_r, rumtfLabel_t label, int *rc);

/**
 * Indicates that all flows should be accepted on the queue_r, i.e., all the messages
 * received will be delivered. A call to this function resets the list of accepted
 * flows that may have been built earlier for this queue_r (using the rumRAddAcceptedFlow API).
 * Any subsequent call to one of the TurboFlow label APIs invalidates the accept-all mode.
 * This function can be applied only to TurboFlow queues or streamsets.
 * If the application is busy processing messages of the queue_r the function may return RUM_BUSY
 * indicating that operation cannot be completed.In this case the application should try to call this
 * function again.
 * @param queue_r An initialized receiver queue or streamset set to work in TurboFlow Label mode (RUM_TF_MODE_LABEL).
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success (start accepting all), RUM_BUSY if the operation cannot be completed due to messages
 * (of queue_r) being processed by the application, or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRAcceptAllFlows(const rumQueueR *queue_r, int *rc);

/**
 * Indicates that all flows should be rejected on the queue_r, i.e., no messages will be delivered.
 * A call to this function resets the list of accepted flows that may have been built earlier for this
 * queue_r (using the rumRAddAcceptedFlow() API).
 * Any subsequent call to one of the TurboFlow label APIs invalidates the reject-all mode.
 * This function can be applied only to TurboFlow queues or streamsets.
 * If the application is busy processing messages of the queue_r the function may return RUM_BUSY
 * indicating that operation cannot be completed.In this case the application should try to call this
 * function again.
 * @param queue_r An initialized receiver queue or streamset set to work in TurboFlow Label mode (RUM_TF_MODE_LABEL).
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success (start rejecting all), RUM_BUSY if the operation cannot be completed due to messages
 * (of queue_r) being processed by the application, or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRRejectAllFlows(const rumQueueR *queue_r, int *rc);

/**
 * Set the message selector that is used to filter messages when the queue is configured to filter messages using an rumMsgSelector.
 * This call can be invoked only on a queue_r that was created with the enable_msg_properties flag set.
 * This call can be invoked more than once. Every call replaces the currently set message selector.
 * It is legal to set the selector to NULL which means that all messages should be accepted.
 * If the application is busy processing messages of the queue_r the function may return RUM_BUSY
 * indicating that operation cannot be completed. In this case the application should try to call this
 * function again.
 * @param queue_r An initialized Receiver queue or streamset created with enable_msg_properties set.
 * @param selector The new message selector to use (input)
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success (selector set), RUM_BUSY if the operation cannot be completed due to messages
 * (of queue_r) being processed by the application, or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRSetMsgSelector(const rumQueueR *queue_r, rumMsgSelector *selector, int *rc);
 ...
 * or after a packet has been received using rumRRecievePacket() and before it is returned using rumRReturnPacket().
 * <p>
 * If the hdr_len field in the rumRxMessage structure is zero, there are no properties and this
 * function does not need to be called.
 *
 * @param msg    A receive message structure
 * @param avail  The number of properties available.  This may be larger than the input count. (output)
 * @param props  An array of message properties. (output)
 * @param count  The number of entries in the props array. (input)
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRGetMsgProperties(rumRxMessage * msg, int * avail, rumMsgProperty * props, int count, int * rc);

/**
 * Get Statistics of a receiver queue.
 * <br> See rumRQueueStats <br>
 * @param queue_r An initialized receiver queue
 * @param report Pointer to rumRQueuestats structure that will be populated (output)
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRGetQueueStatistics(const rumQueueR *queue_r, rumRQueueStats *report, int *rc);

/**
 * Reset running counters in the the statistics of the receiver queue and the statistics of all the active streams
 * accepted by this queue receiver.
 * The fields in rumRQueueStats which are reset by this API are documented as maintaining the counter  
 * "since the receiver queue was created or the queue or receiver instance statistics were reset", e.g., trimmed_packets.
 * @param queue_r An initialized receiver queue
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRResetQueueStatistics(const rumQueueR *queue_r, int *rc);

/* Latency Monitoring related Functions */

/**
 * This API is used to set latency monitoring parameters on a transmitter queue.
 * <br> See rumLatencyMonParams <br>
 * Note that the sampling rate for latency monitoring (i.e., the number of packets per second for which latency is calculated) is
 * set by an advanced configuration parameter called LatencyMonSampleRate. The value of LatencyMonSampleRate is for the entire transmitter
 * instance. LatencyMonSampleRate is set to zero by default which means that RUM will set the sampling rate automatically based on the
 * monitoring level and the cost associated with each time sample.
 * This API can be invoked more than once in order to change the latency monitoring parameters. It is important to note
 * that the change in the parameters does not happen immediately; it may take RUM some time (typically no more than a few milliseconds)
 * before it adjusts to the new settings.
 * It is expected that after the first time this API is called to set the initial parameters it will be used mainly to change the
 * monitoring level.
 * @param queue_t  An initialized Transmitter queue structure.
 * @param mon_params - Initialized rumLatencyMonParams structure with the requested latency monitoring information.
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumTSetLatencyMonitoringParams(const rumQueueT *queue_t, const rumLatencyMonParams *mon_params, int *rc) ;

/**
 * This API is used to retrieve latency monitoring information of a transmitter queue.
 * <br> See rumTxLatencyInfo   <br>
 * @param queue_t  An initialized Transmitter queue structure.
 * @param latency_info - An rumTxLatencyInfo structure to hold the latency monitoring information.
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumTGetLatencyInfo(const rumQueueT *queue_t, rumTxLatencyInfo *latency_info, int *rc) ;


/**
 * This API is used to set latency monitoring parameters on a receiver queue.
 * <br> See rumLatencyMonParams   <br>
 * Note that the sampling rate for latency monitoring (i.e., the number of packets per second for which latency is calculated) is
 * set by an advanced configuration parameter called LatencyMonSampleRate. The value of LatencyMonSampleRate is for the entire receiver
 * instance. LatencyMonSampleRate is set to zero by default which means that RUM will set the sampling rate automatically based on the
 * monitoring level and the cost associated with each time sample.
 * This API can be invoked more than once in order to change the latency monitoring parameters. It is important to note
 * that the change in the parameters does not happen immediately; it may take RUM some time (typically no more than a few milliseconds)
 ...
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRSetLatencyMonitoringParams(const rumQueueR *queue_r, const rumLatencyMonParams *mon_params, int *rc) ;

/**
 * This API is used to retrieve latency monitoring information of a receiver queue.
 * <br> See rumRxLatencyInfo   <br>
 * @param queue_r  An initialized Receiver queue structure.
 * @param latency_info - An rumRxLatencyInfo structure to hold the latency monitoring information.
 * @param rc A pointer to an integer where the failure-specific error code will be copied (output)
 * @return RUM_SUCCESS on success or RUM_FAILURE on failure.
 */
RUMAPI_DLL int  rumRGetLatencyInfo(const rumQueueR *queue_r, rumRxLatencyInfo *latency_info, int *rc) ;
