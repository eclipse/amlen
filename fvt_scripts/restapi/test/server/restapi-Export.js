...
var verifyENUM = [ "ConfigurationPolicy", "CertificateProfile", "SecurityProfile", "ClientCertificate", 
...
var verifyUnchangedENUM = [ "AdminUserID", "AdminEndpoint", "TraceLevel", "LicensedUsage"  ];
...
var enableServerConfig = '{'+ configServerName +', '+ configClusterMembership +', '+ configHighAvailability   +', '+ configTolerateRecoveryInconsistencies   +'}';
...
// 10.a. IMPORT
// 10.b. Verify GET Updated Config
...
// POST UPDATE CONFIG
    ...
    // Update with the Batch Post
        postUpdatedConfig();
        it('should return status 200 GET HA Config ', function(done) {
            verifyConfig = JSON.parse( '{'+ configHighAvailability +'}' )  ;
            FVT.makeGetRequest( FVT.uriConfigDomain +"HighAvailability", FVT.getSuccessCallback, verifyConfig, done)
        });        
        it('should return status 200 GET ClusterMembership Config ', function(done) {
            verifyConfig = JSON.parse( '{'+ configClusterMembership +'}' )  ;
            FVT.makeGetRequest( FVT.uriConfigDomain +"ClusterMembership", FVT.getSuccessCallback, verifyConfig, done)
        });        
...
// EXPORT THE CONFIG for later
	describe('Export Configuration:', function() {

        it('should return status 200 POST "Export" config', function(done) {
            this.timeout( FVT.REBOOT + 3000 );
            var payload = '{ "FileName":"ExportServerConfig", "Password":"ima4test" }';
            verifyConfig = JSON.parse( FVT.expectAllConfigedStatus ) ;
            FVT.makePostRequest( FVT.uriServiceDomain+"export", payload, FVT.postSuccessCallback, verifyConfig, done);
        });
        it('should return status 200 on GET "Status" after export', function(done) {
            verifyConfig = JSON.parse( '{ "status":400, "Code":"CWLNA0137", "Message":"The REST API call: /ima/'+ FVT.version +'/service/export is not valid." }' ) ;
            FVT.makeGetRequest( FVT.uriServiceDomain+"export", FVT.getFailureCallback, verifyConfig, done)
        });
// BAD DOMAIN
        it('should return 400 POST "Export" to config DOMAIN', function(done) {
            var payload = '{ "Password":"ima4test" }';
            verifyConfig = JSON.parse( FVT.expectAllConfigedStatus ) ;
			verifyMessage = JSON.parse( '{ "status":400, "Code":"CWLNA0137", "Message":"The REST API call: /ima/'+ FVT.version +'/configuration/export is not valid." }' );
            FVT.makePostRequestWithVerify( FVT.uriConfigDomain+"export", payload, FVT.postFailureCallback, verifyConfig, verifyMessage, done);
        });

// Missing Required Parameter
        it('should return 200 POST "Export" defaultname imaBackup.<serverUID>.timestamp', function(done) {
            var payload = '{ "FileName":"Password Is Required" }';
            verifyConfig = JSON.parse( FVT.expectAllConfigedStatus ) ;
			verifyMessage = { "status":400, "Code":"CWLNA0134","Message":"The value specified for the required property is invalid or null. Property: Password Value: null." } ;
            FVT.makePostRequestWithVerify( FVT.uriServiceDomain+"export", payload, FVT.postFailureCallback, verifyConfig, verifyMessage, done);
        });

// Invalid Required Parameter (name too long)
        it('should return 400 POST "Export" defaultname imaBackup.<serverUID>.timestamp', function(done) {
            var payload = '{ "FileName":"'+ FVT.long1025String +'" }';
            verifyConfig = JSON.parse( FVT.expectAllConfigedStatus ) ;
			verifyMessage = { "status":400, "Code":"CWLNA0134","Message":"The value specified for the required property is invalid or null. Property: Password Value: null." } ;
            FVT.makePostRequestWithVerify( FVT.uriServiceDomain+"export", payload, FVT.postFailureCallback, verifyConfig, verifyMessage, done);
        });
...
// Remaining Steps are done outside this test case, see ism-RESTAPI-01.sh
//  ----------------------------------------------------------------------
// Save the EXPORTED file
// RESET CONFIG  -- done outside this 
