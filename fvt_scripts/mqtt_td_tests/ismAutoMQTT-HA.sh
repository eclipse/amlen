...
# Turn on saving of passed logs. Need to see everything for some of the problems going on here
setSavePassedLogs on ${logfile}
setComponentSleep default ${logfile}
totalResults=0
setSavePassedLogs off ${logfile}
export start_imaserver_timeout=900
...
RUNCMD="../scripts/run-scenarios.sh  HA/ism-mqtt_td-HASetup.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

RUNCMD="../scripts/run-scenarios.sh  HA/ism-mqtt_td-HAScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

RUNCMD="../scripts/run-scenarios.sh  HA/ism-mqtt_td-HAScenarios02.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

RUNCMD="../scripts/run-scenarios.sh  HA/ism-mqtt_td-HAScenarios03.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

RUNCMD="../scripts/run-scenarios.sh  HA/ism-mqtt_td-HAScenarios04.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

RUNCMD="../scripts/run-scenarios.sh  HA/ism-mqtt_td-HAScenarios05.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
...
RUNCMD="../scripts/run-scenarios.sh  HA/ism-mqtt_td-HAScenarios06.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
...
RUNCMD="../scripts/run-scenarios.sh  HA/ism-mqtt_td-HAScenarios07.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )


# IoTP Monitoring Cleanup tests
setSavePassedLogs on ${logfile}

mqttv5_shared_subs/updateAllowMqttProxyProtocol.sh on
mqttv5_shared_subs/updateMqttProxyMonitoring.sh on

RUNCMD="../scripts/run-scenarios.sh  HA/ism-mqtt_td-HAScenarios08.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

mqttv5_shared_subs/updateMqttProxyMonitoring.sh off
mqttv5_shared_subs/updateAllowMqttProxyProtocol.sh off
...
RUNCMD="../scripts/run-scenarios.sh  HA/ism-mqtt_td-HAScenarios09.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
...
# Hack to make sure containers are running before disable HA is started.
appliance=1
...
do
    ...
    aSrvContIDVal=$(eval echo \$${aSrvContID})
    
    #ssh ${aUserVal}@${aIPVal} docker start ${aSrvContIDVal}
    ../scripts/serverControl.sh -a startServer -i 1
    
    ((appliance+=1))
done

RUNCMD="../scripts/haFunctions.py  -a disableHA -f disableHA.log"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
