...
RUNCMD="../scripts/run-scenarios.sh  ism-plugin-setupScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

RUNCMD="../scripts/run-scenarios.sh  subscribe/ism-plugin_td-subscribeScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

RUNCMD="../scripts/run-scenarios.sh  retain/ism-plugin_td-retainScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

RUNCMD="../scripts/run-scenarios.sh  ism-plugin-teardownScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

RUNCMD="../scripts/run-scenarios.sh  restmsg/ism-restplugin-setupScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

RUNCMD="../scripts/run-scenarios.sh  restmsg/ism-restplugin-testScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
...
# MOCHA RESULTS #
#################
passing=`grep " passing (" ism-restplugin-testScenarios01/restmsg.log | sed -e 's/^[ \t]*//' | cut -d' ' -f1`
failing=`grep " failing" ism-restplugin-testScenarios01/restmsg.log | sed -e 's/^[ \t]*//' | cut -d' ' -f1`
if [[ ${failing} == "" ]]; then
    failing=0
fi
TOTAL=`expr ${passing} + ${failing}`
echo -e "\nTest Result of MessageSight REST Message Plugin Tests: 0 (Total:${TOTAL} Pass:${passing} Fail:${failing})\n" | tee -a ${logfile}
#################

RUNCMD="../scripts/run-scenarios.sh  restmsg/ism-restplugin-teardownScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
