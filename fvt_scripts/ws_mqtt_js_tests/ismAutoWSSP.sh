...
#
# Description:
#   This script executes the ISM test bucket for:  WebSocket SubProtocols.
#
# Input Parameters:
...
# If Appliance does not have local LDAP then we need to start the LDAP on M1 and configure Appliance LDAP object
if [[ "$A1_LDAP" == "FALSE" ]] ; then
  RUNCMD="../scripts/run-scenarios.sh  ws_m1ldap_setup_runScenarios.sh ${logfile}"
  echo Running command: ${RUNCMD}
  $RUNCMD
  totalResults=$( expr ${totalResults} + $? )
fi

## Publish/Subscribe - MQTT JSON
#RUNCMD="../scripts/run-scenarios.sh  jsclient_pubsub/ism-jsclient_pubsub-runScenarios01.sh ${logfile}"
#echo Running command: ${RUNCMD}
#$RUNCMD
#totalResults=$( expr ${totalResults} + $? )

# Connect - MQTT Binary
RUNCMD="../scripts/run-scenarios.sh  jsclient_connect_b/ism-jsclient_connect-runScenarios01_b.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

# Security - MQTT Binary
RUNCMD="../scripts/run-scenarios.sh  jsclient_sec_b/ism-jsclient_sec-runScenarios01_b.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

# Clean Session - MQTT Binary
#RUNCMD="../scripts/run-scenarios.sh  jsclient_clean_b/ism-jsclient_clean-runScenarios01_b.sh ${logfile}"
#echo Running command: ${RUNCMD}
#$RUNCMD
#totalResults=$( expr ${totalResults} + $? )

# Publish/Subscribe - MQTT Binary
RUNCMD="../scripts/run-scenarios.sh  jsclient_pubsub_b/ism-jsclient_pubsub-runScenarios01_b.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
...
if [[ "$A1_LDAP" == "FALSE" ]] ; then
  RUNCMD="../scripts/run-scenarios.sh  ws_m1ldap_cleanup_runScenarios.sh ${logfile}"
  echo Running command: ${RUNCMD}
  $RUNCMD
  totalResults=$( expr ${totalResults} + $? )
fi
