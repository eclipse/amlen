...
# TODO: Replace this when we are capable of updating AllowSingleNIC property
./HA/updateAllowSingleNIC.sh

# Test Bucket Scenarios (group 1)
RUNCMD="../scripts/run-scenarios.sh  HA/ism-JMS-HASetup-00.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

# Test Bucket Scenarios (group 2)
RUNCMD="../scripts/run-scenarios.sh  HA/ism-JMS-HARetained-01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

# Test Bucket Scenarios (group 3)
RUNCMD="../scripts/run-scenarios.sh  HA/ism-JMS-HABasic-02.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

# Test Bucket Scenarios (group 3)
RUNCMD="../scripts/run-scenarios.sh  HA/ism-JMS-HASamples-01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
...
if [[ "${FULLRUN}" == "TRUE" ]] ; then
	...
	RUNCMD="../scripts/run-scenarios.sh  HA/ism-JMS-HAResync-03.sh ${logfile}"
	echo Running command: ${RUNCMD}
	$RUNCMD
	totalResults=$( expr ${totalResults} + $? )
fi
...
RUNCMD="../scripts/run-scenarios.sh  HA/ism-JMS-HACleanup-04.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

# Test Bucket Scenarios (group 6)
RUNCMD="../scripts/run-scenarios.sh  expiry/ism-JMS-ExpiryHA-00.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

# Test Bucket Scenarios (group 7)
#RUNCMD="../scripts/run-scenarios.sh  admin_dynamic/ism-JMS-AdminDynamicHA-00.sh ${logfile}"
#echo Running command: ${RUNCMD}
#$RUNCMD
#totalResults=$( expr ${totalResults} + $? )
...
RUNCMD="../scripts/run-scenarios.sh  HA/ism-JMS-DisableHA-05.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
