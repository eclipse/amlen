...
scenario_set_name="JMS MaxMessageBehavior - 00"
...
#----------------------------------------------------
# Due to a change in the JMS client made in July 2019
#   to the fix atomicity of the cache size count, these tests no longer work.
#
# According to Ken, they were counting on an inaccurate behavior of the JMS cache
#
...
#----------------------------------------------------
# Due to a change in the JMS client made in July 2019
#   to the fix atomicity of the cache size count, this test no longer works.
#
# According to Ken, they were counting on an inaccurate behavior of the JMS cache
#
...
#----------------------------------------------------
# Scenario 4 - jms_maxmsgs_004
#----------------------------------------------------
xml[${n}]="jms_maxmsgs_004"
timeouts[${n}]=60
scenario[${n}]="${xml[${n}]} - Shared JMS Subscription exceeding MaxMessage size discardOldest, lots of consumers. "
component[1]=sync,m1
component[2]=jmsDriver,m1,maxmsgbehavior/${xml[${n}]}.xml,Cons1
component[3]=jmsDriver,m1,maxmsgbehavior/${xml[${n}]}.xml,Prod1
component[4]=jmsDriver,m1,maxmsgbehavior/${xml[${n}]}.xml,Prod2
components[${n}]="${component[1]} ${component[2]} ${component[3]} ${component[4]}"
((n+=1))
...
if [[ ${SLOWDISKPERSISTENCE} == "Yes" ]]  ; then
	...
	xml[${n}]="jms_maxmsgs_005Virtual"
	timeouts[${n}]=100
	scenario[${n}]="${xml[${n}]} -DISKPERSISENCE VERSION of Durable Shared JMS Subscription exceeding MaxMessage size discardOldest, lots of consumers. "
	component[1]=sync,m1
	component[2]=jmsDriver,m1,maxmsgbehavior/${xml[${n}]}.xml,Cons1
	component[3]=jmsDriver,m1,maxmsgbehavior/${xml[${n}]}.xml,Prod1
	component[4]=jmsDriver,m1,maxmsgbehavior/${xml[${n}]}.xml,Prod2
	...
	xml[${n}]="jms_maxmsgs_005AsyncVirtual"
	timeouts[${n}]=100
	scenario[${n}]="${xml[${n}]} - Durable Shared JMS Subscription exceeding MaxMessage size discardOldest, lots of consumers. "
	component[1]=sync,m1
	component[2]=jmsDriver,m1,maxmsgbehavior/${xml[${n}]}.xml,Cons1
	component[3]=jmsDriver,m1,maxmsgbehavior/${xml[${n}]}.xml,Prod1
	component[4]=jmsDriver,m1,maxmsgbehavior/${xml[${n}]}.xml,Prod2
