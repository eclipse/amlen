...
#----------------------------------------------------
# Test Case 0 - Change cluster sync interval
# 
# This should include putting some retained messages in the cluster. 
#----------------------------------------------------
xml[${n}]="testmqtt_ClusterSyncIntervals"
timeouts[${n}]=30
scenario[${n}]="${xml[${n}]} - Modify cluster sync intervals for test"
...
component[1]=cAppDriverLogWait,m1,"-e|../scripts/run-cli.sh","-o|-s|setupC1|-c|common_topic_tree/clusterCTT_config.cli"
component[2]=cAppDriverLogWait,m1,"-e|../scripts/run-cli.sh","-o|-s|setupC2|-c|common_topic_tree/clusterCTT_config.cli|-a|2"
components[${n}]="${component[1]} ${component[2]}"
((n+=1))
