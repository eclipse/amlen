...
RUNCMD="../scripts/run-scenarios.sh  ism-TestBucketStub-runScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

## Test Bucket Scenarios (group 2)  Repeat as many groups necessary
# RUNCMD="../scripts/run-scenarios.sh  ism-TestBucketStub-runScenarios02.sh ${logfile}"
# echo Running command: ${RUNCMD}
# $RUNCMD
#totalResults=$( expr ${totalResults} + $? )?
