...
totalResults=0
export IMAPort=16102
#----------------------------------------------------
# Removed Test Scripts: CommitTest, PtPListenerTest
...
#RUNCMD="../scripts/run-scenarios.sh  ism-CommitTest-runScenarios01.sh ${logfile}"
...
#RUNCMD="../scripts/run-scenarios.sh  ism-PtPListenerTest-runScenarios01.sh ${logfile}"
#echo Running command: ${RUNCMD}
#$RUNCMD
#totalResults=$( expr ${totalResults} + $? )
...
if [[ "${A1_LOCATION}" == "remote" ]] ; then
	...
	RUNCMD="../scripts/run-scenarios.sh  ism-TopicListenerTestREMOTE-runScenarios01.sh ${logfile}"
	echo Running command: ${RUNCMD}
	$RUNCMD
	totalResults=$( expr ${totalResults} + $? )
else 
	# Test Bucket Scenarios (group 2)
	RUNCMD="../scripts/run-scenarios.sh  ism-TopicListenerTest-runScenarios01.sh ${logfile}"
	echo Running command: ${RUNCMD}
	$RUNCMD
	totalResults=$( expr ${totalResults} + $? )
fi
...
# Test Bucket Scenarios (group 4)  Repeat as many groups necessary
export component_sleep=0
if [[ "${A1_LOCATION}" == "remote" ]] ; then
	# Test Bucket Scenarios (group 2 BASIC tests only) 
	RUNCMD="../scripts/run-scenarios.sh  ism-TemporaryTestREMOTE-runScenarios01.sh ${logfile}"
	echo Running command: ${RUNCMD}
	$RUNCMD
	totalResults=$( expr ${totalResults} + $? )
else 
	# Test Bucket Scenarios (group 2)
	RUNCMD="../scripts/run-scenarios.sh  ism-TemporaryTest-runScenarios01.sh ${logfile}"
	echo Running command: ${RUNCMD}
	$RUNCMD
	totalResults=$( expr ${totalResults} + $? )
fi
...
# Test Bucket Scenarios (group 5)  Repeat as many groups necessary
export component_sleep=0
RUNCMD="../scripts/run-scenarios.sh  ism-SimplePubSubTest-runScenarios01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
unset component_sleep
