...
#
#  !!! NOTE: These tests require the appliances to have an HA1 and HA2 NIC. !!!
#      So they are not run on baremetal HA automation
#
#
# Input Parameters:
...
RUNCMD="../scripts/run-scenarios.sh  ./haSimplifiedTests.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

# Test Bucket Scenarios (group 2)
#RUNCMD="../scripts/run-scenarios.sh  ./haFunctionsTests.sh ${logfile}"
#echo Running command: ${RUNCMD}
#$RUNCMD
#totalResults=$( expr ${totalResults} + $? )

# Test Bucket Scenarios (group 3)
RUNCMD="../scripts/run-scenarios.sh  ./haGVTTests.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
