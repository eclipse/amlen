...
#
# Description:
#   This script executes ISM test bucket for:  IMA MQTT BRIDGE: HA, PLain and Proxy.
#
# start:  from ../$TESTROOT/bridge_tests/   run:  ismAutoBridge_ALL.sh <log_file>
#
# Input Parameters:
...
then
  ...
  dos2unix proxy/*
  dos2unix *.sh 
  dos2unix ../scripts/st*MQTTBridge.sh 
  dos2unix ../common/Bridge.*.cfg
fi  
...
# Update server to use proxy mqtt protocol
../common/serverProxyConfig.sh on


# Allow SingleNIC to handle both HA services
...
# PLAIN Functions
RUNCMD="../scripts/run-scenarios.sh  plain/ism-Bridge-Plain01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )

# HA Functions 
RUNCMD="../scripts/run-scenarios.sh  ha/ism-Bridge-HA01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
...
RUNCMD="../scripts/run-scenarios.sh  proxy/ism-Bridge-Proxy01.sh ${logfile}"
echo Running command: ${RUNCMD}
$RUNCMD
totalResults=$( expr ${totalResults} + $? )
...
# Update server to not use proxt mqtt protocol
