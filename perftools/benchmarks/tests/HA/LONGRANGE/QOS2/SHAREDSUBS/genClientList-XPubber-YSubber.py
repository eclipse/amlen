...
# the HA.LONGRANGE benchmark test.
#
# Variant:
...
######################################################################################
CLIENTLIST_FILE_PREFIX="HA-LONGRANGE-Q2-SSUBS-"
NUMSUBAPPS = 1
PERFAPP="perfapp"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q2.SSUBS variant of the HA.LONGRANGE benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip, deleteState):
    ...
    topic = MBTopic("halongrange/q2-ssubs-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/" + client._id)
    topic.qos = 2
...
def createSUBClient(id, version, dstip, appid, deleteState):
    ...
    sub = MBTopic("$share/" + appname + "/halongrange/q2-ssubs-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/#")
    sub.qos = 2
...
for i in range(0, int(NUMSUBCLIENTS)):
    ...
    client = createSUBClient(i, MBCONSTANTS.MQTT_TCP5, dstip, appid, False)
    clientlist.append(client)
    
    clientDeleteState = createSUBClient(i, MBCONSTANTS.MQTT_TCP5, dstip, appid, True)
    clientDeleteStateList.append(clientDeleteState)
