...
######################################################################################
CLIENTLIST_FILE_PREFIX="HA-LONGRANGE-Q0-SSUBS-"
NUMSUBAPPS = 1
PERFAPP="perfapp"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q0.SSUBS variant of the HA.LONGRANGE benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip):
    ...
    topic = MBTopic("halongrange/q0-ssubs-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/" + client._id)
    topic.qos = 0
...
def createSUBClient(id, version, dstip, appid):
    ...
    sub = MBTopic("$share/" + appname + "/halongrange/q0-ssubs-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/#")
    sub.qos = 0
