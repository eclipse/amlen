...
if [ "$1" == "" -o "$2" == "" -o "$3" == "" -o "$4" == "" ] ; then
    echo "usage: $0 <number of publishers (in SI units)> <number of subscribers (in SI units)> <msg size in bytes> <remote data center>"
    echo "  e.g. $0 1M 1K 64 dal12"
    echo "  e.g. $0 100K 1K 64 wdc06"
    exit 1
else
    NUMPUBBERS=$1
    NUMSUBBERS=$2
