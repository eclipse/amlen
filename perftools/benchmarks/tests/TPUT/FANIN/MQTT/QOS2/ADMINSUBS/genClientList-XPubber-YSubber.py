...
#
# Variant:
# - QoS 2 (non-durable client sessions)
# - Topic tree is partitioned into non-overlapping segments
# - For each partition an administrative subscription is created and for each 
#   administrative subscription there are multiple shared subscribers.
# - X publishers  (each publishing on a unique topic)
# - Y subscribers (single administrative subscription per partition)
#
# NOTE: X and Y are based on the parameters passed to this script. Both X and Y are in 
#       SI units, e.g. 1, 500, 1K, 500K, 1M, etc.
######################################################################################
CLIENTLIST_FILE_PREFIX="TPUT-FANIN-MQTT-Q2-ADMINSUBS-"
PERFAPP="perfapp"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q1.ADMINSUBS variant of the TPUT.FANIN.MQTT benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip, partition):
    ...
    topic = MBTopic("mqttfanin/q2-adminsubs/part" + str(partition) + "/" + client._id)
    topic.qos = 2
...
def createSUBClient(id, version, dstip, partition):
    ...
    sub = MBTopic("$share/" + appname + "/mqttfanin/q2-adminsubs/part" + str(partition) + "/#")
    sub.qos = 2
...
for i in range(0, int(NUMPUBCLIENTS)):
    ...
    client = createPUBClient(i, MBCONSTANTS.MQTT_TCP5, dstip, partition)
...
for i in range(0, int(NUMSUBCLIENTS)):
    ...
    partition = i % NUMPARTITIONS
    client = createSUBClient(i, MBCONSTANTS.MQTT_TCP5, dstip, partition)
    clientlist.append(client)
    count += 1
