...
######################################################################################
CLIENTLIST_FILE_PREFIX="TPUT-FANIN-MQTT-Q0-PARTITION-"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q0.PARTITION variant of the TPUT.FANIN.MQTT benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip, partition):
    ...
    topic = MBTopic("mqttfanin/q0-partition-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/part" + str(partition) + "/" + client._id)
    topic.qos = 0
...
def createSUBClient(id, version, dstip, partition):
    ...
    sub = MBTopic("mqttfanin/q0-partition-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/part" + str(partition) + "/#")
    sub.qos = 0
