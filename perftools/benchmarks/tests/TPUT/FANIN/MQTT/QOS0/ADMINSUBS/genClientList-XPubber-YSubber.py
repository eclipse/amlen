...
######################################################################################
CLIENTLIST_FILE_PREFIX="TPUT-FANIN-MQTT-Q0-ADMINSUBS-"
PERFAPP="perfapp"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q0.ADMINSUBS variant of the TPUT.FANIN.MQTT benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip, partition):
    ...
    topic = MBTopic("mqttfanin/q0-adminsubs/part" + str(partition) + "/" + client._id)
    topic.qos = 0
...
def createSUBClient(id, version, dstip, partition):
    ...
    sub = MBTopic("$share/" + appname + "/mqttfanin/q0-adminsubs/part" + str(partition) + "/#")
    sub.qos = 0
