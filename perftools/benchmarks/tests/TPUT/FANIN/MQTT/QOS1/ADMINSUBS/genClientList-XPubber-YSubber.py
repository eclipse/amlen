...
#
# Variant:
# - QoS 1 (non-durable client sessions)
...
######################################################################################
CLIENTLIST_FILE_PREFIX="TPUT-FANIN-MQTT-Q1-ADMINSUBS-"
PERFAPP="perfapp"
...
def createPUBClient(id, version, dstip, partition):
    ...
    topic = MBTopic("mqttfanin/q1-adminsubs/part" + str(partition) + "/" + client._id)
    topic.qos = 1
...
def createSUBClient(id, version, dstip, partition):
    ...
    sub = MBTopic("$share/" + appname + "/mqttfanin/q1-adminsubs/part" + str(partition) + "/#")
    sub.qos = 1
