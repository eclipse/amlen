...
NUMSUBBERS=0
if [ "$1" == "" -o "$2" == "" -o "$3" == "" ] ; then
    echo "usage: $0 <number of publishers (in SI units)> <number of subscribers (in SI units)> <msg size in bytes>"
    echo "  e.g. $0 1M 1 64"
    echo "  e.g. $0 100K 1K 64"
    exit 1
else
    NUMPUBBERS=$1
    NUMSUBBERS=$2
...
export TESTVARIANT=${TESTNAME}-Q1-ADMINSUBS-${NUMPUBBERS}PUB-${NUMSUBBERS}SUB
