...
######################################################################################
CLIENTLIST_FILE_PREFIX="TPUT-FANIN-MQTT-Q1-SSUBS-"
NUMSUBAPPS = 1
PERFAPP="perfapp"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q1.SSUBS variant of the TPUT.FANIN.MQTT benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip, deleteState):
    ...
    topic = MBTopic("mqttfanin/q1-ssubs-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/" + client._id)
    topic.qos = 1
...
def createSUBClient(id, version, dstip, appid, deleteState):
    ...
    sub = MBTopic("$share/" + appname + "/mqttfanin/q1-ssubs-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/#")
    sub.qos = 1
