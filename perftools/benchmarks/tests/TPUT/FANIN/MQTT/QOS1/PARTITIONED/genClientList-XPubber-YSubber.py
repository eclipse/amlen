...
######################################################################################
CLIENTLIST_FILE_PREFIX="TPUT-FANIN-MQTT-Q1-PARTITION-"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q1.PARTITION variant of the TPUT.FANIN.MQTT benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip, partition, deleteState):
    ...
    topic = MBTopic("mqttfanin/q1-partition-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/part" + str(partition) + "/" + client._id)
    topic.qos = 1
...
def createSUBClient(id, version, dstip, partition, deleteState):
    ...
    sub = MBTopic("mqttfanin/q1-partition-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/part" + str(partition) + "/#")
    sub.qos = 1
