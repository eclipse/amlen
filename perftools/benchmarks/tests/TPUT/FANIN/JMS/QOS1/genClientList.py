...
# the TPUT.FANIN.JMS benchmark test.
#
# Variant:
# - QoS 1 (durable client sessions)
# - JMS durable, shared subscriptions with client ACK
...
######################################################################################
CLIENTLIST_FILE_PREFIX="TPUT-FANIN-JMS-Q1-SSUBS-"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q1.SSUBS variant of the TPUT.FANIN.JMS benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip, deleteState):
    ...
    topic = MBTopic("jmsfanin/q1-jms/" + client._id)
    topic.qos = 1
