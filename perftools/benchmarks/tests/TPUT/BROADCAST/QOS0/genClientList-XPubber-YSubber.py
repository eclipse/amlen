...
######################################################################################
CLIENTLIST_FILE_PREFIX="TPUT-BROADCAST-Q0-"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q0 variant of the TPUT.BROADCAST benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip, topicNum):
    ...
    topic = MBTopic("broadcast/q0-" + NUMTOPICSTRING + "TOPICS-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/broadcast" + str(topicNum))
    topic.qos = 0
...
def createSUBClient(id, version, dstip, topicNum):
    ...
    sub = MBTopic("broadcast/q0-" + NUMTOPICSTRING + "TOPICS-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/broadcast" + str(topicNum))
    sub.qos = 0
...
for i in range(0, int(NUMPUBCLIENTS)):
    ...
    client = createPUBClient(i, MBCONSTANTS.MQTT_TCP5, dstip, topicNum)
...
for i in range(0, int(NUMSUBCLIENTS)):
    ...
    topicNum = count % NUMTOPICS
    client = createSUBClient(i, MBCONSTANTS.MQTT_TCP5, dstip, topicNum)
    clientlist.append(client)
    count += 1
