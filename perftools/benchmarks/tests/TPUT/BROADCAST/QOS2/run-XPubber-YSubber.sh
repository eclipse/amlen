...
if [ "$1" == "" -o "$2" == "" -o "$3" == "" -o "$4" == "" ] ; then
    echo "usage: $0 <number of publishers (in SI units)> <number of subscribers (in SI units)> <number of topics> <msg size in bytes> [deletestate]"
    echo "  e.g. $0 1 1K 1 64"
    echo "  e.g. $0 1 10K 1 4096 deletestate"
    exit 1
else
    NUMPUBBERS=$1
    NUMSUBBERS=$2
...
export TESTVARIANT=${TESTNAME}-Q2-${NUMTOPICS}TOPICS-${NUMPUBBERS}PUB-${NUMSUBBERS}SUB
export TESTVARIANT_DS=${TESTNAME}-Q2-${NUMTOPICS}TOPICS-${NUMPUBBERS}PUB-${NUMSUBBERS}SUB-DELETESTATE
