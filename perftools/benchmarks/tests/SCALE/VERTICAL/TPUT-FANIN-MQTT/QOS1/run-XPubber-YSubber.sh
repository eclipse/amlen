...
# -----------------------------------------------------------------------------------------------------
#
# This script executes the following variant of the SCALE.VERTICAL.TPUT.FANIN.MQTT benchmark test.
#
# Variant:
...
# - DelayTime environment variable (based on CPU count)
#
...
# - the number of CPU used to run MessageSight server
...
if [ "$1" == "" -o "$2" == "" -o "$3" == "" -o "$4" == "" ] ; then
    echo "usage: $0 <number of publishers (in SI units)> <number of subscribers (in SI units)> <msg size in bytes> <# CPU on MessageSight server> [deletestate]"
    echo "  e.g. $0 1M 100 64 4"
    echo "  e.g. $0 1M 100 64 4 deletestate"
    exit 1
else
    NUMPUBBERS=$1
    NUMSUBBERS=$2
...
export TESTNAME=SCALE-VERT-TPUT-FANIN-MQTT
export TESTVARIANT=${TESTNAME}-Q1-HYBRID-${NUMPUBBERS}PUB-${NUMSUBBERS}SUB
export TESTVARIANT_DS=${TESTNAME}-Q1-HYBRID-${NUMPUBBERS}PUB-${NUMSUBBERS}SUB-DELETESTATE
export GraphiteMetricRoot=$TESTVARIANT-MSG-${MSGSIZE}-CPU-${NUMCPU}

# delay between new connections (units in microseconds)
export DelayTime=$((1800 / ($NUMCPU / 4)))
export DelayCount=1
...
if [ "$DELETESTATE" == "1" ] ; then
	...
	mqttbench -cl $TESTVARIANT.json -r 1 -s ${MSGSIZE}-${MSGSIZE} -T 0x1 -snap connects,msgrates,msgcounts,msglatency 4 -u 1e-6 -csv $TESTVARIANT-MSG-${MSGSIZE}-CPU-${NUMCPU}.stats.csv -lcsv $TESTVARIANT-MSG-${MSGSIZE}-CPU-${NUMCPU}.latency.csv -rl 300 -st 3 
fi
