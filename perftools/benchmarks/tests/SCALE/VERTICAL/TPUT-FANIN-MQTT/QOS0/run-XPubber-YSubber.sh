...
if [ "$1" == "" -o "$2" == "" -o "$3" == "" -o "$4" == "" ] ; then
    echo "usage: $0 <number of publishers (in SI units)> <number of subscribers (in SI units)> <msg size in bytes> <# CPU on MessageSight server>"
    echo "  e.g. $0 1M 100 64 4"
    echo "  e.g. $0 10K 1K 64 8"
    exit 1
else
    NUMPUBBERS=$1
    NUMSUBBERS=$2
...
export TESTVARIANT=${TESTNAME}-Q0-HYBRID-${NUMPUBBERS}PUB-${NUMSUBBERS}SUB
