...
###################################################################################### 
#
# This script generates the mqttbench client list for running the 
# SCALE-VERTICAL-TPUT-FANIN-MQTT benchmark test.
#
# Variant:
...
######################################################################################
CLIENTLIST_FILE_PREFIX="SCALE-VERT-TPUT-FANIN-MQTT-Q0-HYBRID-"
PERFAPP="perfapp"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q0.HYBRID variant of the SCALE.VERTICAL.TPUT.FANIN.MQTT benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip, partition):
    ...
    topic = MBTopic("scale-vertical-tput/q0-hybrid-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/part" + str(partition) + "/" + client._id)
    topic.qos = 0
...
def createSUBClient(id, version, dstip, partition):
    ...
    sub = MBTopic("$share/" + appname + "/scale-vertical-tput/q0-hybrid-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/part" + str(partition) + "/#")
    sub.qos = 0
