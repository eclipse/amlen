...
######################################################################################
CLIENTLIST_FILE_PREFIX="SCALE-VERT-TPUT-FANIN-MQTT-Q2-HYBRID-"
PERFAPP="perfapp"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the Q2.HYBRID variant of the SCALE.VERTICAL.TPUT.FANIN.MQTT benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createPUBClient(id, version, dstip, partition, deleteState):
    ...
    topic = MBTopic("scale-vertical-tput/q2-hybrid-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/part" + str(partition) + "/" + client._id)
    topic.qos = 2
...
def createSUBClient(id, version, dstip, partition, deleteState):
    ...
    sub = MBTopic("$share/" + appname + "/scale-vertical-tput/q2-hybrid-" + NUMPUBSTRING + "PUB-" + NUMSUBSTRING + "SUB/part" + str(partition) + "/#")
    sub.qos = 2
