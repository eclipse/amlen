...
# MessageSight server provisioned with 4 CPUs. The following settings are calibrated for a 4 CPU
# server:
#
...
# Reset connection latency statistics every 3960 seconds (-rl 3960) and latency histogram units are in milliseconds (-cu 1e-3).  
# Clients remain connected for 3960 seconds (-l 3960)
mqttbench -cl $TESTNAME.json -T 0x20 -snap connects,connlatency 4 -cu 1e-3 -csv $TESTVARIANT.stats.csv -lcsv $TESTVARIANT.latency.csv -rl 3960 -l 3960 
