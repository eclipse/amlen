...
# Reset connection latency statistics every 960 seconds (-rl 960) and latency histogram units are in milliseconds (-cu 1e-3).  
# Clients remain connected for 960 seconds (-l 960)
mqttbench -cl $TESTNAME.json -T 0x20 -snap connects,connlatency 4 -cu 1e-3 -csv $TESTVARIANT.stats.csv -lcsv $TESTVARIANT.latency.csv -rl 960 -l 960 
