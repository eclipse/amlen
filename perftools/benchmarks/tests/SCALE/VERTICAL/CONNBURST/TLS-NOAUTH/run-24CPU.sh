...
# Reset connection latency statistics every 310 seconds (-rl 310) and latency histogram units are in milliseconds (-cu 1e-3).  
# Clients remain connected for 310 seconds (-l 310)
mqttbench -cl $TESTNAME.json -T 0x20 -snap connects,connlatency 4 -cu 1e-3 -csv $TESTVARIANT.stats.csv -lcsv $TESTVARIANT.latency.csv -rl 310 -l 310 
