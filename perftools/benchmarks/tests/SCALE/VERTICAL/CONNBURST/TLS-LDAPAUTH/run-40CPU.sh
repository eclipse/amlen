...
# Reset connection latency statistics every 270 seconds (-rl 270) and latency histogram units are in milliseconds (-cu 1e-3).  
# Clients remain connected for 270 seconds (-l 270)
mqttbench -cl $TESTNAME.json -T 0x20 -snap connects,connlatency 4 -cu 1e-3 -csv $TESTVARIANT.stats.csv -lcsv $TESTVARIANT.latency.csv -rl 270 -l 270 
