...
# MessageSight server provisioned with 56 CPUs. The following settings are calibrated for a 56 CPU
# server:
#
...
export TESTVARIANT=${TESTNAME}-56CPU
...
# Reset connection latency statistics every 200 seconds (-rl 200) and latency histogram units are in milliseconds (-cu 1e-3).  
# Clients remain connected for 200 seconds (-l 200)
mqttbench -cl $TESTNAME.json -T 0x20 -snap connects,connlatency 4 -cu 1e-3 -csv $TESTVARIANT.stats.csv -lcsv $TESTVARIANT.latency.csv -rl 200 -l 200 
