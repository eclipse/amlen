...
# Reset connection latency statistics every 540 seconds (-rl 540) and latency histogram units are in milliseconds (-cu 1e-3).  
# Clients remain connected for 540 seconds (-l 540)
mqttbench -cl $TESTNAME.json -T 0x20 -snap connects,connlatency 4 -cu 1e-3 -csv $TESTVARIANT.stats.csv -lcsv $TESTVARIANT.latency.csv -rl 540 -l 540 
