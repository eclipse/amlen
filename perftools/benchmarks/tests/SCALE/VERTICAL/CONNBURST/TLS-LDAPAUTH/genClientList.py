...
# SCALE-VERTICAL-CONNBURST-TLS-LDAPAUTH benchmark test.
#
######################################################################################
CLIENTLIST_FILE="SCALE-VERTICAL-CONNBURST-TLS-LDAPAUTH.json"
NUMCLIENTS = 1e6
USERNAME_PREFIX="h"
PASSWORD="svtpvtpass"
...
def readArgs():
    ...
    parser = argparse.ArgumentParser(description='Creates the mqttbench client list file for the SCALE.VERTICAL.CONNBURST.TLS.LDAPAUTH benchmark', formatter_class=argparse.RawTextHelpFormatter)
...
def createClient(id, version, dstip):
    ...
    client.useTLS = True
    client.username = "{0}{1}".format(USERNAME_PREFIX, str(id).zfill(8))
