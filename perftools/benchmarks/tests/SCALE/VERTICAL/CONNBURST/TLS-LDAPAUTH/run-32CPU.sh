...
# MessageSight server provisioned with 32 CPUs. The following settings are calibrated for a 32 CPU
# server:
#
...
# Reset connection latency statistics every 290 seconds (-rl 290) and latency histogram units are in milliseconds (-cu 1e-3).  
# Clients remain connected for 290 seconds (-l 290)
mqttbench -cl $TESTNAME.json -T 0x20 -snap connects,connlatency 4 -cu 1e-3 -csv $TESTVARIANT.stats.csv -lcsv $TESTVARIANT.latency.csv -rl 290 -l 290 
