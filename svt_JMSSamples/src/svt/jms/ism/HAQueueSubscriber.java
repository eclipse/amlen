...
/**
 * Standalone queue subscriber sample for high availability
 * 
 * This application is intended for use with HAQueuePublisher.  Both applications
 ...
 * 3. Queue name - the name of the queue from which messages will be received
 *                 NOTE: This Queue name must match the name used for the HAQueuePublisher sample
 ...
 * 
 * IMPORTANT NOTE:   THIS DOES NOT APPLY TO QUEUEs  
 * This sample does not remove the durable subscription so that messages sent by HATopicPublisher 
 ...
 *      java com.ibm.ima.samples.jms.HAQueueSubscriber "server1.mycompany.com, server2.mycompany.com" 16102 Queue 2000 clientID
 *      java com.ibm.ima.samples.jms.HAQueueSubscriber "server1.mycompany.com, server2.mycompany.com" 16102 Queue 2000 clientID  60
 * 
 * To see the usage statement for the program, run it with no arguments:
 *      java com.ibm.ima.samples.jms.HAQueueSubscriber
 * 
 */
public class HAQueueSubscriber implements Runnable {
    ...
    static String queuename = null;             /* The Queue name for the subscription. */
    ...
    public static void syntaxhelp(String msg) {
        System.err.println("java HAQueueSubscriber  serverlist serverport queuename count [receivetimeout] [msgwaittimeout] [forcetimeout] [connecttimeout] [clientID]");
		System.err.println("  Example:");
        System.err.println("      java HAQueueSubscriber \"server1.mycompany.com, server2.mycompany.com\" 16102 queue 2000 30");
        System.err.println("      java HAQueueSubscriber \"server1.mycompany.com, server2.mycompany.com\" 16102 queue 2000 30 60 120 120 clientID ");
        if (msg != null)
            System.err.println("\n" + msg);
        System.exit(1);
    }
     ...
     *      queuename           subscription Queue name
    ...
    public static void main(String[] args) {
         ...
         */
        new Thread(new HAQueueSubscriber()).start();
         ...
         */
        synchronized (HAQueueSubscriber.class) {
...
//        }
    ...
    public static void parseArgs(String[] args) {
        ...
        /* 
         * The name of the subscription Queue is the third required argument. This name must match the Queue
         * name specified in HAQueuePublisher.
         */
        queuename = args[2];
    ...
    public static void doCheckDone() {
        ...
        while (!done) {
            synchronized (HAQueueSubscriber.class) {
                if (!recvdone) {
                    try {
                        HAQueueSubscriber.class.wait(500);
                    } catch (Exception e) {
                    }
    ...
    public void doReceive() throws JMSException {       
    	Queue theQueue = getQueue();
    	cons = sess.createConsumer(theQueue);
    	conn.start();
    	int i, firstmsg;
