 ...
 * Where:
 *
 * TrustedCertificate         : Trusted Certificate file name.
 * SecurityProfileName        : Name of the security profile.
 *
 *
 */

XAPI int32_t ism_config_validate_TrustedCertificate(json_t *currPostObj, json_t *validateObj, char * item, char * name, int action, ism_prop_t *props)
{
    int32_t rc = ISMRC_OK;
    ...
    if ( json_typeof(validateObj) == JSON_NULL ) {
		...
		ism_common_setErrorData(ISMRC_DeleteNotAllowed, "%s", "TrustedCertificate");
		goto VALIDATION_END;
    }
    ...
    filelist = alloca(sizeof(char *) * json_array_size(mergedObj));
    ...
    for (i=0; i<json_array_size(mergedObj); i++) {
		...
		while(itemIter)
		{
	        ...
	        if ( !strcmp(key, "TrustedCertificate")) {
				...
				trustCert = ism_common_strdup(ISM_MEM_PROBE(ism_memory_admin_misc,1000),(char *)json_string_value(value));
				if ( trustCert && *trustCert != '\0' ) {
					rc = ism_config_validateItemData( reqList, (char *)key, (char *)trustCert, action, props);
					if ( rc != ISMRC_OK ) {
						goto VALIDATION_END;
					}
				}else {
		...
		int ncerts = 0;
		rc = ism_config_checkTrustedCertExist(0, spName, trustCert, &isNewFile, &ncerts);
		if (rc) {
			goto VALIDATION_END;
		}
		...
		if (isNewFile) {
			if (overwrite)
				...
				rc = ism_admin_ApplyCertificate("TRUSTED", "true", (char *)spName, (char *)trustCert, NULL, NULL);
			else
				rc = ism_admin_ApplyCertificate("TRUSTED", "false", (char *)spName, (char *)trustCert, NULL, NULL);
			...
			if (rc) {
				...
				rc = xrc;
				if ( rc == 6184 ) {
					ism_common_setErrorData(rc, "%s", trustCert?trustCert:"");
				}
				goto VALIDATION_END;
			}

			//add the new file into filelist for later deleting
            int flen = strlen(trustCert);
			char *newfile = alloca(flen + 1);
			memset(newfile, 0, flen+1);
			memcpy(newfile, trustCert, flen);
			newfile[flen] = '\0';
		...
		if (trustCert)  ism_common_free(ism_memory_admin_misc,trustCert);
