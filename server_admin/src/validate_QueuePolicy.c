 ...
 *
 * Description:
 * 		Policy to validate queue messaging activity
 * 		QueuePolicy cannot be deleted if it is in use by an Endpoint object
 *
 * Schema:
 * {
 * 		"QueuePolicy": {
 ...
 *			"Protocol":"string",
 *			"Queue":"string",
 *			"ActionList":"string", 			(LIST: Send,Receive,Browse)
 *			"MaxMessageTimeToLive":"string",
 *		}
 *	}
 *
 *
 * Description: Description of the queue policy object. Optional Item.
 ...
 * Queue: match for destination topics to use this policy
 * ActionList: Enum - Send,Receive,Browse
 ...
 *
 */
XAPI int32_t ism_config_validate_QueuePolicy(json_t *currPostObj, json_t *mergedObj, char * item, char * name, int action, ism_prop_t *props)
{
    int32_t rc = ISMRC_OK;
    ...
    while(itemIter)
    {
 	...
 	/* QueuePolicy can not be deleted if used by an endpoint */
	if (action == 2) {
	    rc = ism_config_valDeleteEndpointObject("QueuePolicies", name);
