...
/**
 * Returns the file contents of the specified file in /tmp/userfiles dir
 */
...
/**
 * Returns the list of files and dir in /tmp/userfiles dir
 */
XAPI int ism_config_json_getFileList(ism_http_t *http, char *fname) {
    int rc = ISMRC_OK;

    struct dirent *dent;
    char *basedir = "/tmp/userfiles";

    DIR * bDir = opendir(basedir);

    if (bDir == NULL) {
    	TRACE(3, "Could not open /tmp/userfiles directory. errno:%d\n", errno);
    	rc = ISMRC_NotFound;
    	ism_common_setError(rc);
    	return rc;
    }
