...
/* Validator for ClusterRequestedTopics */
 ...
 * Where:
 *
 * TopicString          : Topic string to requested from all cluster members
 *
...
XAPI int32_t ism_config_validate_ClusterRequestedTopics(json_t *currPostObj, json_t *validateObj, char * item, char * name, int action, ism_prop_t *props)
{
    int32_t rc = ISMRC_OK;
    ...
    if ( json_typeof(validateObj) == JSON_NULL ) {
		...
		ism_common_setErrorData(ISMRC_DeleteNotAllowed, "%s", "ClusterRequestedTopics");
		goto VALIDATION_END;
    }
    ...
    for (i=0; i<json_array_size(mergedObj); i++) {
        ...
        if ( propValue && *propValue != '\0' ) {
			...
			if (rc != ISMRC_OK) {
				...
				ism_common_setErrorData(rc, "%s%s", "ClusterRequestedTopics", propValue);
			}
        }else {
