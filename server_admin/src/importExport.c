...
XAPI int ism_config_json_parseServiceImportPayload(ism_http_t *http, ism_rest_api_cb callback) {
    ...
    char filePath[2048];
    sprintf(filePath, USERFILES_DIR "/.v2Config");
    ...
    if ( rc == ISMRC_OK ) {
        if ( processV2Config == 1 ) {
            ...
            if ( cfgFile == 1 ) {
                sprintf(filePath, USERFILES_DIR "/%s", fileName);
            } else {
            	sprintf(filePath, USERFILES_DIR "/%s.tmpdir" IMA_SVR_INSTALL_PATH "/config/server_dynamic.cfg", fileName);
            }
			...
			if ( cfgFile == 0 && rc == ISMRC_OK && newConfigRoot != NULL ) {
				...
				sprintf(filePath, USERFILES_DIR "/%s.tmpdir" IMA_SVR_INSTALL_PATH "/config/mqcbridge_dynamic.cfg", fileName);
				newConfigRoot = ism_config_migrate_propsToJSON(filePath, newConfigRoot, &rc);
			}
			if ( newConfigRoot ) {
	            if ( cfgFile == 1 ) {
