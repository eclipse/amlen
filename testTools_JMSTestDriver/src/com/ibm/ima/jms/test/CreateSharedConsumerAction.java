...
public class CreateSharedConsumerAction extends ApiCallAction {
	...
	protected CreateSharedConsumerAction(Element config, DataRepository dataRepository, TrWriter trWriter) {
	...
	protected boolean invokeApi() throws JMSException {
		...
		if (_selector == null)
			consumer = ((ImaSubscription)ses).createSharedConsumer(topic, _durableName);
		else
			consumer = ((ImaSubscription)ses).createSharedConsumer(topic, _durableName, _selector);
