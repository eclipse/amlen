...
IMA_PROXY_PATH_DEFINES := $(shell python3 $(PRJDIR)/$(ROOTREL)/server_build/path_parser.py -p IMA_PROXY -p IMA_PRODUCT  -m=CFLAGS -s=info)

IMA_SVR_AND_BRIDGE_PATH_DEFINES := $(IMA_SVR_PATH_DEFINES) $(IMA_BRIDGE_PATH_DEFINES) 
IMA_SVR_AND_BRIDGE_AND_PROXY_PATH_DEFINES := $(IMA_SVR_PATH_DEFINES) $(IMA_BRIDGE_PATH_DEFINES) $(IMA_PROXY_PATH_DEFINES)
...
IDFLAGS = -DISMDATE=`date +%Y-%m-%d` -DISMTIME=`date +%H:%M` -DBUILD_LABEL=$(BUILD_LABEL) -DBUILD_ID=$(BUILD_ID) -DISM_VERSION="$(ISM_VERSION)" -DIMA_SOURCELEVEL_INFO="$(IMA_SOURCELEVEL_INFO)"
