...
##################################################################################
cancelRunningJobs(){
  if [ "${USER}" == "${STREAMS_UID}" ]; then
      RUNNING_JOBS=`streamtool lsjob  --xheaders  -i ${STREAMS_INSTANCE} `
  else
  ...
  do
    ...
    if [ "${USER}" == "${STREAMS_UID}" ]; then
       streamtool  canceljob ${JOB_ID}  -i ${STREAMS_INSTANCE}
    else
...
if [ "${USER}" == "${STREAMS_UID}" ]; then
   RUNNING_INSTANCES=`streamtool lsinstance --started `
else
...
if [[ ${RUNNING_INSTANCES} =~ .*${STREAMS_INSTANCE}.* ]];then
  ...
  if [ "${USER}" == "${STREAMS_UID}" ]; then
     START_INSTANCE=`streamtool startinstance -i ${STREAMS_INSTANCE} `
  else
...
echo " "
echo "Compiling SPL Application in ${PROJECT_DIR}"
cd ${PROJECT_DIR}
${STREAMS_INSTALL}/bin/sc -M MqttGreenThread --output-directory=output/MqttGreenThread/Distributed --data-directory=data -t ${STREAMS_INSTALL}/toolkits/com.ibm.streams.messaging: --no-toolkit-indexing --no-mixed-mode-preprocessing 
if [ $? -ne 0 ]; then
  echo "ERROR Compiling ${PROJECT_DIR}, Aborting!"
  exit 66
else
  echo "Successfully compiled ${PROJECT_DIR}."
  echo " "
fi

#Start the SPL Application
if [ "${USER}" == "${STREAMS_UID}" ]; then
   SUBMIT_JOB=`streamtool submitjob  -i ${STREAMS_INSTANCE}  --P '${TESTCASE_PARM1}'  --outfile ${JOB_ID_FILE}  ${TESTCASE_ADL} `
else
...
if [ "${USER}" == "${STREAMS_UID}" ]; then
   streamtool canceljob  -i ${STREAMS_INSTANCE}  -f  ${JOB_ID_FILE} 
   streamtool stopinstance  -i ${STREAMS_INSTANCE}
else 
