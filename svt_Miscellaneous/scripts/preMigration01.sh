...
scenario_set_name="Misc Premigration Scenarios 01"
source ../scripts/commonFunctions.sh

test_template_set_prefix "miscPreMig_01_"
...
#----------------------------------------------------
 ...
 printf -v tname "miscPreMig_%02d" ${n}
 ...
 test_template_define "testcase=${tname}" "description=mqtt_41_311_HA" "timeout=600" "order=true" "fill=true" "failover=0" "verify=false" "userid=false" "${client[*]}" "${count[*]}" "${action[*]}" "${id[*]}" "${qos[*]}" "${durable[*]}" "${persist[*]}" "${topic[*]}" "${port[*]}" "${messages[*]}" "$( IFS=$'|'; echo "${message[*]}" )" "${rate[*]}" "${ssl[*]}"
