...
scenario_set_name="Misc SVT Scenarios 03"
source ../scripts/commonFunctions.sh

test_template_set_prefix "misc_03_"
...
#----------------------------------------------------
#
#  This testcase stores 2,000,000 messages in store, verifies messages
#
#----------------------------------------------------
  printf -v tname "misc_03_%02d" ${n}
...
if [ "${A1_TYPE}" == "DOCKER" ]; then
  ...
  declare sub=$((${pub}*${M_COUNT}))
  declare x=0

  ((x=0)); client[$x]=jmqtt; count[$x]=1;action[$x]=subscribe;id[$x]=10;qos[$x]=2;durable[$x]=true ;persist[$x]=false;topic[$x]=/svtGroup0/chat;port[$x]=16212;messages[$x]=${sub};message[$x]='Message Order Pass';rate[$x]=0   ;ssl[$x]=false;
  ((x++)); client[$x]=jmqtt; count[$x]=${M_COUNT};action[$x]=publish  ;id[$x]=20;qos[$x]=2;durable[$x]=false;persist[$x]=true ;topic[$x]=/svtGroup0/chat;port[$x]=16211;messages[$x]=${pub};message[$x]=`eval echo 'Published ${pub} messages'`;rate[$x]=4000;ssl[$x]=false;

  test_template_define "testcase=${tname}" "description=`eval echo 'Stores $((${pub}*${M_COUNT})) messages in store verifies messages'`" "timeout=${TIMEOUT}" "order=true" "fill=true" "failover=1" "verify=true" "userid=true" "wait=after" "${client[*]}" "${count[*]}" "${action[*]}" "${id[*]}" "${qos[*]}" "${durable[*]}" "${persist[*]}" "${topic[*]}" "${port[*]}" "${messages[*]}" "$( IFS=$'|'; echo "${message[*]}" )" "${rate[*]}" "${ssl[*]}"
