...
# MessageSight command line interface to Test an Objections Connectivity Status
...
def main(argv):
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description='Test an Objections Connectivity Status' )
    parser.add_argument('-s', metavar='adminServerIP:port', required=True, type=str, help='adminServerIP:port')
    parser.add_argument('object', metavar='ConfigObject', type=str, help='Configuration object', choices=['LogAutoTransfer', 'LDAP', 'QueueManagerConnection'] )
    ...
    # Set payload
    payload_str = '{"Locale":"en_US","User":"admin","Action":"purge","Item":"' + objectType + '" }'
    trace( 'CMD: \n'+ payload_str )
    ...
    rc = parseResponse( 'Test', adminServer, objectType, response )
