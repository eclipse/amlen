...
# MessageSight command line interface to APPLY Certificates
...
#    parser = argparse.ArgumentParser(description='Get configuration object', epilog=usage())
    parser = argparse.ArgumentParser(description='Apply Certificate objects' )
    parser.add_argument('-s', metavar='adminServerIP:port', required=True, type=str, help='adminServerIP:port')
    parser.add_argument('object', metavar='ConfigObject', type=str, help='Configuration object', choices=['Certificate', 'PreSharedKey'] )
    ...
    if objectType == 'File' :
        ...
        payload_str = '{"Locale":"en_US","User":"admin","Action":"apply","Item":"' + objectType + '"'+ param_str +' }'
    trace( 'CMD: \n'+ payload_str )
    ...
    rc = parseResponse( 'Apply', adminServer, objectType, response )
