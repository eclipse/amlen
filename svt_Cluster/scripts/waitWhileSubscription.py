...
#----------------------------------------------------------------------------
def get(endpoint, clientid, start, wait, maxwait):
   data = ''
...
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
def process(comparison, data, value, field, maxwait, wait, endpoint, clientid, start):
   ...
   if (( comparison == "-lt" ) or ( comparison == "lt" )):
      ...
      while (( int(data['Subscription'][0][field]) < int(value) ) and ((now-start) < int(maxwait) )):
         ...
         print "time.sleep(" + wait + ")"
         time.sleep(float(wait))
         data = get(endpoint, clientid, start, wait, maxwait)
         print 'wait while {0}({1})<{2}'.format(field,data['Subscription'][0][field],value)
         now = time.time()
...
def main(argv):
   ...
   try:
      options, optargs = getopt.getopt(argv,"a:i:c:v:w:m:k:r:h",["endpoint1=","clientid=","comparison=","value=","field=","wait=","maxwait=","retry="])
   ...
   if 'Subscription' in data:
      if len(data['Subscription']) > 0:
         if field in data['Subscription'][0]:
            process(comparison, data, value, field, maxwait, wait, endpoint, clientid, start)
         else:
