...
scenario_set_name="Cluster Scenarios 04"

test_template_set_prefix "cluster_04_"
...
printf -v tname "cluster_04_%02d" ${n}
xml[${n}]=${tname}
test_template_initialize_test "${xml[${n}]}"
scenario[${n}]="${xml[${n}]} - Fan Out of ${MESSAGES} messages"
...
for i in $(eval echo {2..${M_COUNT}}); do
   j=1; while [ $j -lt ${A_COUNT} ]; do
      ...
      component[$((++lc))]="javaAppDriver,m$i,-e|svt.mqtt.mq.MqttSample,-o|-a|subscribe|-mqtt|3.1.1|-q|${QOS}|-t|/svtGroup0/chat|-s|tcp://${AP}:${SPORT}+tcp://${AS}:${SPORT}|-i|${id}|-u|${id}|-p|imasvtest|-c|false|-O|-e|true|-n|${MESSAGES}|-Nm|${MINUTES}|-receiverTimeout|0:0"
...
printf -v id "u%07d" ${count}
component[$((++lc))]="javaAppDriver,m1,-e|svt.mqtt.mq.MqttSample,-o|-a|publish|-mqtt|3.1.1|-q|${QOS}|-t|/svtGroup0/chat|-s|tcp://${A1_IPv4_1}:${PPORT}+tcp://${A2_IPv4_1}:${PPORT}|-i|${id}|-u|${id}|-p|imasvtest|-O|-n|${MESSAGES}|-w|500"
