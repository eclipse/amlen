...
public class ImaTopicToMqTopicTwoQmsOneQMCTest extends ImaTopicToMqDestination {

	public ImaTopicToMqTopicTwoQmsOneQMCTest(String clientID) {
		super();
	...
	static ImaTopicToMqTopicTwoQmsOneQMCTest test;
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		try {
			test = new ImaTopicToMqTopicTwoQmsOneQMCTest("ImaT2MqT2QmsTest");
			test.setNumberOfMessages(5000);
			test.setNumberOfPublishers(1);
			test.setNumberOfSubscribers(1);
			test.topicString = "ISMTopic/To/MqTopic/TwoQmsOneQMC";
			test.setSecondmqChannel("JAVA.CHANNEL");
			test.setSecondQueueManagerName("dttest3");
			test.setSecondmqPort(1418);
			...
			// now check that all messages came from 1 sub - there should not be any on the other
			if (noOfMessagesFromSub1==0 && noOfMessagesFromSub2 ==0) {
				// no messages received - failed
				System.out.println("TEST FAILED - No Messages");	
				test.setSuccess(false);
			} else if (noOfMessagesFromSub1==0 || noOfMessagesFromSub2 ==0) {
				// test OK
			} else {
				// messages on both. test failed
				System.out.println("TEST FAILED - Messages on both subs");	
				test.setSuccess(false);
			}		
		} catch (MQException e) {
