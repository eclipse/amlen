...
public class MqQueueMqJavaToImaTopicMqtt extends MqQueueToImaDestination {

	public MqQueueMqJavaToImaTopicMqtt() {
		super();
		this.testName = "MqQueueToImaTopic";
		this.clientID = "MqQueueToImaTopic";
	}

	public static void main(String[] args) {
		boolean success = MqQueueMqJavaToImaTopicMqtt.run(args);
		if (success==false) {
	...
	public static boolean run(String[] args) {
		...
		for (int x : mqPersistences) {
			for (int qos=0; qos <=2; qos++) {
				MqQueueMqJavaToImaTopicMqtt test = null;
				try {
					test = new MqQueueMqJavaToImaTopicMqtt();
					test.setNumberOfMessages(5);
					test.setNumberOfPublishers(1);
					test.setNumberOfSubscribers(1);
					test.mqQueueName = "MQQueue.To.ISMTopic";
					test.topicString = "MQQueue/To/ISMTopic";
					test.waitTime =10000;
					if (args.length > 0) {
