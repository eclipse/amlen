...
/**
 * Test 8 - MQ Topic to IMA Topic
 * 
 * This test will set up a destination mapping rule from an MQ topic
 * to an IMA topic. It will update the 'RetainedMessages' value from 'None'
...
public class Retained_IMAQueueToMQTopic extends JmsToJms {
	
	static EnabledEditConfig config;
	static String testName = "Retained_MQQueueToIMATopic";
	...
	public static void main(String[] args) {
		...
		Retained_IMAQueueToMQTopic testClass = new Retained_IMAQueueToMQTopic();
	 ...
	 * Constructors
	 */
	public Retained_IMAQueueToMQTopic(){
		//TODO - Fill this in (with what?)
	}
	
	public Retained_IMAQueueToMQTopic(EnabledEditConfig configObject){
		config = configObject;
	}
	...
	public void startTest(){
		try {
			...
			if(!config.mqConn.mappingRuleExists(testName)){
				...
				config.mqConn.createMappingRule(testName, config.prefixName, "11", imadestinationName, mqDestinationName);
				if(config.mqConn.mappingRuleExists(testName)){
			...
			if(!config.config.queueExists(imadestinationName)){
				Trace.trace("Setting up IMA Queue");
				config.config.createQueue(imadestinationName, "IMAQueue", 5000);
				if(config.config.queueExists(imadestinationName)){
					Trace.trace("IMA Queue created");
				} else {
					Trace.trace("Could not create IMA Queue");
				}
			} else {
				Trace.trace("IMA Queue already exists");
			}
			...
			//Set up topic, queue and message
...
/*				config.teardown();
		...
		try {			
			...
			if(config.config.queueExists(imadestinationName)){
				Trace.trace("Deleting IMA Queue");
				config.config.deleteQueue(imadestinationName, true);
				if(!config.config.queueExists(imadestinationName)){
					Trace.trace("IMA Queue deleted");
				}
			}
