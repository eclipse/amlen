...
public class Retained_MQQueueToIMATopic extends JmsToJms {
	...
	public static void main(String[] args) {
		...
		Retained_MQQueueToIMATopic testClass = new Retained_MQQueueToIMATopic();
	 ...
	 * Constructors
	 */
	public Retained_MQQueueToIMATopic(){
		//TODO - Fill this in (with what?)
	}
	
	public Retained_MQQueueToIMATopic(EnabledEditConfig configObject){
		config = configObject;
	}
	...
	public void startTest(){
		try {
			...
			if(!config.mqConn.mappingRuleExists(testName)){
				...
				config.mqConn.createMappingRule(testName, config.prefixName, "3", mqDestinationName, imadestinationName);
				if(config.mqConn.mappingRuleExists(testName)){
			...
			consumer1 = consumerSession.createConsumer(imaDestination);
			producer.send(msg);
			...
			consumer2 = consumerSession.createConsumer(imaDestination);
