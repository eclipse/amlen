...
/**
 * Test 5 - IMA Topic Subtree to MQ Topic Subtree
 * 
 * This test will set up a destination mapping rule from an IMA topic subtree
...
public class Retained_IMATopicSubtreeToMQTopicSubtree extends JmsToJms{
	
	static EnabledEditConfig config;
	static String testName = "Retained_IMATopicSubtreeToMQTopicSubtree";
	
	public static void main(String[] args) {
		
		new Trace().traceAutomation();
		Retained_IMATopicSubtreeToMQTopicSubtree testClass = new Retained_IMATopicSubtreeToMQTopicSubtree();
	 ...
	 * Constructers
	 */
	public Retained_IMATopicSubtreeToMQTopicSubtree(){
		//TODO - Fill this in (with what?)
		new Trace().traceAutomation();
	}
	
	public Retained_IMATopicSubtreeToMQTopicSubtree(EnabledEditConfig configObject){
		new Trace().traceAutomation();
		config = configObject;
	...
	public void startTest(){
		try {
			...
			if(!config.mqConn.mappingRuleExists(testName)){
				...
				config.mqConn.createMappingRule(testName, config.prefixName, "7", imadestinationName, mqDestinationName);
				if(config.mqConn.mappingRuleExists(testName)){
			...
			Topic imaDestinationSubtree1 = producerSession.createTopic(imadestinationName + "/LEVEL1");
			Topic imaDestinationSubtree2 = producerSession.createTopic(imadestinationName + "/LEVEL1/LEVEL2");
			
			Topic mqDestination =  consumerSession.createTopic(mqDestinationName);
			Topic mqDestinationSubtree1 =  consumerSession.createTopic(mqDestinationName + "/LEVEL1");
			Topic mqDestinationSubtree2 =  consumerSession.createTopic(mqDestinationName + "/LEVEL1/LEVEL2");
			...
			MessageProducer producer1 = producerSession.createProducer(imaDestination);
			MessageProducer producer2 = producerSession.createProducer(imaDestinationSubtree1);
			MessageProducer producer3 = producerSession.createProducer(imaDestinationSubtree2);
			...
			consumer1 = consumerSession.createConsumer(mqDestinationSubtree1);
			producer2.send(msg);
			...
			consumer2 = consumerSession.createConsumer(mqDestinationSubtree1);
			...
			//Send another message
			Trace.trace("Sending another message");
			consumer1 = consumerSession.createConsumer(mqDestinationSubtree2);
			producer3.send(msg);
			...
			Trace.trace("Attempting to receive another message with Consumer2");
			consumer2 = consumerSession.createConsumer(mqDestinationSubtree2);
			receivedMessage2 = null;
			receivedMessage2 = consumer2.receive(5000);
			if (receivedMessage2 == null){
				Trace.trace("No message was received. Pass!");
...
/*			if(isSuccess()){
		...
		try {
			...
			//Trace.trace("Clearing retained publication");
			config.mqConfig.clearRetained(mqDestinationName, mqqueuemanager);
			config.mqConfig.clearRetained(mqDestinationName + "/LEVEL1", mqqueuemanager);
			config.mqConfig.clearRetained(mqDestinationName+ "/LEVEL1/LEVEL2", mqqueuemanager);
