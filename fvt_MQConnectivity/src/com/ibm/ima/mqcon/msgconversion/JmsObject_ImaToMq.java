...
/**
 * Test sends a JMS Object message to IMA topic
 ...
 *
 */
public class JmsObject_ImaToMq extends JmsToJms {
	
	public static void main(String[] args) {
		
		new Trace().traceAutomation();
		JmsObject_ImaToMq testClass = new JmsObject_ImaToMq();
		testClass.setNumberOfMessages(1);
		testClass.setNumberOfPublishers(1);
		testClass.setNumberOfSubscribers(1);
		testClass.setTestName("JmsObject_ImaToMq");
		...
		try
		{
			...
			testClass.sendJMSObjectMessage();
	...
	public void sendJMSObjectMessage()
	{
		try
		{
			if(Trace.traceOn())
			...
			Message msg = JmsMessage.createMessage(producerSession, JmsMessage.MESSAGE_TYPE.OBJECT);
