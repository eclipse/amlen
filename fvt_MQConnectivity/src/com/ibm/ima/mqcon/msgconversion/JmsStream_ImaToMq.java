...
/**
 * Test sends a JMS Stream message to IMA topic
 ...
 *
 */
public class JmsStream_ImaToMq extends JmsToJms {
	
	
	public static void main(String[] args) {
		
		new Trace().traceAutomation();
		JmsStream_ImaToMq testClass = new JmsStream_ImaToMq();
		testClass.setNumberOfMessages(1);
		testClass.setNumberOfPublishers(1);
		testClass.setNumberOfSubscribers(1);
		testClass.setTestName("JmsStream_ImaToMq");
		...
		try
		{
			...
			testClass.sendJMSStreamMessage();
	...
	public void sendJMSStreamMessage()
	{
		try
		{
			if(Trace.traceOn())
			...
			Message msg = JmsMessage.createMessage(producerSession, JmsMessage.MESSAGE_TYPE.STREAM);
