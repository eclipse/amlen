...
/**
 * Test sends a JMS Text message with headers to an MQ topic
 ...
 *
 */
public class JmsHeaders_MqToIma extends JmsToJms{


	public static void main(String[] args) {
		
		new Trace().traceAutomation();
		JmsHeaders_MqToIma testClass = new JmsHeaders_MqToIma();
		testClass.setNumberOfMessages(1);
		testClass.setNumberOfPublishers(1);
		testClass.setNumberOfSubscribers(1);
		testClass.setTestName("JmsHeaders_MqToIma");
	...
	public void sendJMSTextHeaderMessage()
	{
		try
		{
			...
			{
				if(CompareMessageContents.checkJMSMessageEquality(msg, imaMessage, JmsMessage.imaTopic + imadestinationName, JmsMessage.imaTopic))
				{
					if(Trace.traceOn())
					{
