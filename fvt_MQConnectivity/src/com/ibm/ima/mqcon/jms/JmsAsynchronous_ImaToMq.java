...
* and is received from an MQ topic but using
...
*
*/
public class JmsAsynchronous_ImaToMq extends AsynchronousJmsConsumer{
	...
	public static void main(String[] args) {
		...
		JmsAsynchronous_ImaToMq testClass = new JmsAsynchronous_ImaToMq();
		testClass.setNumberOfMessages(4);
		testClass.setNumberOfPublishers(1);
		testClass.setNumberOfSubscribers(1);
		testClass.setTestName("JmsAsynchronous_ImaToMq");
	...
	public void testAsynchronousReceive()
	{
		try
		{
			...
			MessageConsumer consumer = consumerSession.createConsumer(mqDestination, "(_Food$ LIKE 'Ch_ps')");
	...
	public void callback(Message arg0) 
	{
		try
		{
			...
			if(arg0.getStringProperty("_Food$").equals("Chips"))
			{
				...
				if(CompareMessageContents.checkJMSMessageEquality(msg4, arg0, JmsMessage.mqTopic + mqDestinationName, JmsMessage.mqTopic))
				{
					if(Trace.traceOn())
					{
