...
*
*/
public class JmsSelectors_ImaToMq extends JmsToJms{
	...
	public void testSelectors()
	{
		try
		{
			if(Trace.traceOn())
			{
				Trace.trace("Creating the default producer session for IMA and consumer session for MQ");
			}
			...
			if(Trace.traceOn())
			{
				Trace.trace("Setting Food=Pie & Drink=Beer on message1 and Food=Chips & Drink=Beer on message2");
			}
			msg.setStringProperty("Food", "Pie");
			msg.setStringProperty("Drink", "Beer");
			msg2.setStringProperty("Food", "Chips");
			...
			if(Trace.traceOn())
			{
				Trace.trace("Creating the message consumer with the selector (Food = 'Chips') and starting the connection");
			}
			Topic mqDestination = consumerSession.createTopic(mqDestinationName);
			MessageConsumer consumer = consumerSession.createConsumer(mqDestination, "(Food = 'Chips')");
			...
			{
				if(mqMessage.getStringProperty("Food").equals("Chips"))
				{
					if(Trace.traceOn())
					{
						Trace.trace("Message has passed the check for user property 'Food'");
					}
					
					
					if(CompareMessageContents.checkJMSMessagebodyEquality(msg2, mqMessage))
					{
						if(Trace.traceOn())
						{
					...
					else
					{
						if(Trace.traceOn())
						{
							Trace.trace("We received another message which was incorrect. Msg=" + mqMessage2);
						}
				...
				else
				{
					if(Trace.traceOn())
					{
						Trace.trace("The wrong message was returned by the selector. User property for food was expected to be 'Chips' but the actual was " + mqMessage.getStringProperty("Food"));
					}
					RC = ReturnCodes.INCORRECT_MESSAGE;
					closeConnections();
					System.exit(RC);
