...
/*** 
 * Test sends a JMS Text message to MQ topic
* and is received from an IMA topic but using
...
*
*/
public class JmsAsynchronous_MqToIma extends AsynchronousJmsConsumer{
	...
	public static void main(String[] args) {
		...
		JmsAsynchronous_MqToIma testClass = new JmsAsynchronous_MqToIma();
		testClass.setNumberOfMessages(4);
		testClass.setNumberOfPublishers(1);
		testClass.setNumberOfSubscribers(1);
		testClass.setTestName("JmsAsynchronous_MqToIma");
	...
	public void testAsynchronousReceive()
	{
		try
		{
			...
			Session producerSession = producerJMSSession.getSession(JmsSession.CLIENT_TYPE.MQ, mqhostname, mqport, mqqueuemanager, true, Session.AUTO_ACKNOWLEDGE);
			...
			MessageConsumer consumer = consumerSession.createConsumer(imaDestination, "(_Food$ LIKE 'Ch_ps')");
			consumer.setMessageListener(new AsynchronousJmsListener(this));
			consumerJMSSession.startConnection();
			
			if(Trace.traceOn())
			{
				Trace.trace("Sending four messages to MQ");
			}
			producer.send(msg1);
			if(Trace.traceOn())
			{
	...
	public void callback(Message arg0) 
	{
		try
		{
			...
			if(arg0.getStringProperty("_Food$").equals("Chips"))
			{
				...
				if(CompareMessageContents.checkJMSMessageEquality(msg4, arg0, JmsMessage.imaTopic +  imadestinationName, JmsMessage.imaTopic))
				{
					if(Trace.traceOn())
					{
