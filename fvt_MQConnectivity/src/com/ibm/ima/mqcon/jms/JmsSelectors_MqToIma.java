...
*
*/
public class JmsSelectors_MqToIma extends JmsToJms{

	public static void main(String[] args) {
		
		new Trace().traceAutomation();
		JmsSelectors_MqToIma testClass = new JmsSelectors_MqToIma();
		testClass.setNumberOfMessages(2);
		testClass.setNumberOfPublishers(1);
		testClass.setNumberOfSubscribers(1);
		testClass.setTestName("JmsSelectors_MqToIma");
	...
	public void testSelectors()
	{
		try
		{
			...
			MessageConsumer consumer = consumerSession.createConsumer(imaDestination, "(Food = 'Chips')");
			consumerJMSSession.startConnection();
			
			if(Trace.traceOn())
			{
				Trace.trace("Sending both messages to MQ");
			}
			producer.send(msg);
			producer.send(msg2);
			...
			{
				if(imaMessage.getStringProperty("Food").equals("Chips"))
				{
					if(Trace.traceOn())
					{
				...
				else
				{
					if(Trace.traceOn())
					{
						Trace.trace("The wrong message was returned by the selector. User property for food was expected to be 'Chips' but the actual was " + imaMessage.getStringProperty("Food"));
					}
					RC = ReturnCodes.INCORRECT_MESSAGE;
					closeConnections();
					System.exit(RC);
