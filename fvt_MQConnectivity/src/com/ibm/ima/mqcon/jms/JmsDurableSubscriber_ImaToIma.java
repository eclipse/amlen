...
*
*/
public class JmsDurableSubscriber_ImaToIma extends JmsToJms {
	...
	public static void main(String[] args) {
		...
		JmsDurableSubscriber_ImaToIma testClass = new JmsDurableSubscriber_ImaToIma();
		testClass.setNumberOfMessages(2);
		testClass.setNumberOfPublishers(1);
		testClass.setNumberOfSubscribers(1);
		testClass.setTestName("JmsDurableSubscriber_ImaToIma");
		...
		try
		{
			if (args.length==0)
			{
				if(Trace.traceOn())
				{
					Trace.trace("Test aborted, test requires command line arguments!! (IMA IP address, IMA port, IMA topic, timeout)");
				}
	...
	public void testDurability()
	{
		try
		{
			...
			MessageConsumer consumer = consumerSession.createDurableSubscriber(imaDestination2, subID);
			...
			consumer = consumerSession.createDurableSubscriber(imaDestination2, subID);
