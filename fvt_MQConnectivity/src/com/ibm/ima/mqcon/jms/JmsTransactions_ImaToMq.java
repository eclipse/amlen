...
* and is received from an MQ topic when commit is called
...
*
*/
public class JmsTransactions_ImaToMq extends JmsToJms{


	public static void main(String[] args) {
		
		new Trace().traceAutomation();
		JmsTransactions_ImaToMq testClass = new JmsTransactions_ImaToMq();
		testClass.setNumberOfMessages(3);
		testClass.setNumberOfPublishers(1);
		testClass.setNumberOfSubscribers(1);
		testClass.setTestName("JmsTransactions_ImaToMq");
	...
	public void testTransactions()
	{
		try
		{
			...
			Session consumerSession = consumerJMSSession.getSession(JmsSession.CLIENT_TYPE.MQ, mqhostname, mqport, mqqueuemanager, true, Session.AUTO_ACKNOWLEDGE);
			...
			if(Trace.traceOn())
			{
				Trace.trace("Waiting for the message to arrive on MQ");
			}
			...
			{
				if(CompareMessageContents.checkJMSMessagebodyEquality(msg1, mqMessage))
				{
					if(Trace.traceOn())
					{
				...
				else
				{
					...
					{
						if(CompareMessageContents.checkJMSMessagebodyEquality(msg3, mqMessage2))
						{
							if(Trace.traceOn())
							{
