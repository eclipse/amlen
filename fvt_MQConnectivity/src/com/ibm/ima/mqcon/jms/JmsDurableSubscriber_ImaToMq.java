...
*
*/
public class JmsDurableSubscriber_ImaToMq extends JmsToJms {
	
	public static void main(String[] args) {
		
		new Trace().traceAutomation();
		JmsDurableSubscriber_ImaToMq testClass = new JmsDurableSubscriber_ImaToMq();
		testClass.setNumberOfMessages(2);
		testClass.setNumberOfPublishers(1);
		testClass.setNumberOfSubscribers(1);
		testClass.setTestName("JmsDurableSubscriber_ImaToMq");
	...
	public void testDurability()
	{
		try
		{
			...
			Session consumerSession = consumerJMSSession.getSession(JmsSession.CLIENT_TYPE.MQ, mqhostname, mqport, mqqueuemanager, clientID);
			...
			MessageConsumer consumer = consumerSession.createDurableSubscriber(mqDestination, subID);
			...
			consumer = consumerSession.createDurableSubscriber(mqDestination, subID);
