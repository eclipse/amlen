...
#
UNITFILE="${IMA_WEBUI_INSTALL_PATH}/config/imawebui.service"

SCRIPTNAME=$(basename $0)

if [ "$#" -eq "1" ] && [ "$1" == "preset" ]
then
    source ${IMA_WEBUI_INSTALL_PATH}/bin/getUserGroupWebUI.sh
    USERNAME=$IMAWEBUI_USER
    GROUPNAME=$IMAWEBUI_GROUP
elif [ "$#" -eq "2" ]
then
    USERNAME=$1
    GROUPNAME=$2
